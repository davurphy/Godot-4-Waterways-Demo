...
func _init(p_camera, p_shortcut):
	camera = p_camera
	shortcut = p_shortcut
...
func _update_privots(p_privot):
	p_privot.clear()
	p_privot.add_item("None")
	node_list = _get_spatials_recusiv(get_tree().get_root(), [get_name(), camera.get_name()])

	var item_count = node_list.size()
	for i in range(0, item_count):
		var node = node_list[i]
		p_privot.add_item(node.get_name())
