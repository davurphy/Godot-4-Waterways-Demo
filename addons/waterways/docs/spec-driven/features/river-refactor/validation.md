...
- Overall status: Partial — Phases R0 and RT closed; **R1 implemented (R1.1–R1.7, signature now v28)** with the headless gate portion passed 2026-06-12; the human-assisted portion (editor stale warning + rebake round + RT.1 diff against the v27 baseline) is pending
- Last automated pass: 2026-06-12 post-R1 — stale detection demonstrated headless (`SYSTEM_FLOW_COMPARE_STALE` on both demo scenes, rivers skipped as `invalid_flowmap` — the v28 gate firing as designed); suite otherwise green: `BAKE_HASH_PROBE_OK` (v27 baseline readable), `FLOW_SOLVE_SEED_ASSERT_OK`, `DISTMAP_NEUTRAL_BINDING_OK`, `ARROW_NEUTRAL_CELLS_PROBE_OK`, `ARROW_DIRECTION_OUTLIER_PROBE_OK`, `RIVER_FLOWMAP_SEAM_PROBE_OK`, `RIPPLE_MATERIAL_OWNERSHIP_PROBE_OK`, RT.3 `allow_stale=1` report mode exit 0; grep proves zero code references to the deleted SDF-steering set (doc references persist until R8 — expected)
...
- Highest-risk unproven behavior: post-R1 rebake round (user, editor) — editor stale warning visible, rebake of both demo rivers + both system maps, RT.1 diff localization, suite re-green on fresh v28 bakes
...
- RT.3 (after the APPDATA/LOCALAPPDATA redirect from Godot Launch Instructions):
  ...
  - other args: `scene=res://X.tscn stride=4 min_flow=0.05 influence_min=0.05 max_control_deg=15 max_influence_deg=20 height_tol=1.0 allow_stale=1` (stale maps are report-only under `allow_stale=1`, a hard failure otherwise)
