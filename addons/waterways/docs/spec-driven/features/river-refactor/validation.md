...
- R7's accepted compute-default path stays fenced against stale dispatch/readback ordering, reports legacy `R7_TOLERANCE_V1` parity metrics, preserves explicit `legacy_canvas_item` rollback/comparison selection, and limits generated bake output promotion to the accepted source signature v29 scope. Any legacy removal, backend selector collapse, broader saved-output promotion, tolerance change, source signature policy change, or async readback change requires a separate accepted protocol.

## Current Validation Snapshot

- R7 update (2026-06-15): compute-default acceptance is recorded. Validation-only gates, compute backend/projection diagnostics, sampler diagnostics, probe-only `FRAGCOORD` diagnostic, canonical acceptance, generated-output staging, production replacement validation, source signature v29, broader fixture coverage, scoped saved-output promotion, compute-default in-game review, backend performance comparison, and final switched/default acceptance validation are recorded. Latest markers: `R7_COMPUTE_SELECTION_ABORT_OK`, `R7_COMPUTE_NON_NEUTRAL_FLOW_SPEED_OK`, `R7_COMPUTE_SAVED_RESOURCE_LOAD_SMOKE_OK`, `SYSTEM_FLOW_COMPARE_OK`, and `SYSTEM_FLOW_PROJECTED_GATE_OK` under `.codex-research/r7-baselines/compute-switched-default-acceptance/`.

- Overall status: Partial (track) - **Phases R0, RT, R1, R2, R3, R4, R5, R6, R7 compute-default scope, and R8 have implementation/automated validation complete or accepted**. R6.5 cleanup/interface tightening passed focused validation. R7 compute-default acceptance is closed: `canonical_compute_replacing` is the switched/default solve path, scoped saved-output promotion is limited to the two Demo river bake resources, and explicit `legacy_canvas_item` remains available for rollback, comparison, and diagnostics. The optional R6 full-Demo editor undo-delete check was attempted but infeasible due bake load, with lower-cost editor/visible review still optional.
- Last automated/windowed pass: 2026-06-15 R7 switched/default acceptance rerun - Godot 4.6.3 console/windowed with repo-local `APPDATA`/`LOCALAPPDATA`: `R7_COMPUTE_SELECTION_ABORT_OK`, `R7_COMPUTE_NON_NEUTRAL_FLOW_SPEED_OK`, `R7_COMPUTE_SAVED_RESOURCE_LOAD_SMOKE_OK`, `SYSTEM_FLOW_COMPARE_OK`, and `SYSTEM_FLOW_PROJECTED_GATE_OK`.
...
- Highest-risk unproven behavior: legacy-path removal is unapproved. The accepted compute default must keep explicit `legacy_canvas_item` available until a separate side-by-side comparison/removal protocol is defined, run, and accepted. R6 full-Demo editor undo-delete was attempted but infeasible due bake load; any future editor-stack closure needs a lower-cost fixture or targeted harness, and visible review should not be claimed by automated probes.
...
| --- | --- | --- | --- | --- | --- | --- |
| R7 compute-default acceptance | Selection/abort, non-neutral flow-speed, saved-resource load smoke, system-flow compatibility, and projected-flow gate rerun | Windowed RenderingDevice probes plus system-flow checks | Default selects `canonical_compute_replacing`; explicit `legacy_canvas_item` remains available; saved resources load without rebake/save; system-flow compatibility remains within accepted stale-map/report-only bounds | Pass - `R7_COMPUTE_SELECTION_ABORT_OK`, `R7_COMPUTE_NON_NEUTRAL_FLOW_SPEED_OK`, `R7_COMPUTE_SAVED_RESOURCE_LOAD_SMOKE_OK`, `SYSTEM_FLOW_COMPARE_OK`, and `SYSTEM_FLOW_PROJECTED_GATE_OK` under `.codex-research/r7-baselines/compute-switched-default-acceptance/`. This does not approve legacy removal, backend selector collapse, broader saved-output promotion, `R7_TOLERANCE_V1` relaxation, `R7_TOLERANCE_V2`, async readback, or source signature policy changes. | 2026-06-15 | Agent |
...
| R6 behavior preservation | RT.1 hash-compare; abort matrix + metadata dict diffs; R6.5 surface/property check | Headless + rendered console; optional editor/human | Byte-identical bakes; no stuck flags/leaked renderers/orphan viewports; metadata dictionaries diff empty; cleanup preserves public surface/property list | Pass automated - final generated hashes matched post-R6.1H and fresh old-code comparators; canonical dictionaries/API/signal/property matched pre-R6 with only method/signal line numbers normalized and `bake_revision` filtered; R6.5 removed only unreferenced private helpers and preserved surface/property shape; R6.1H abort matrix passed with editor undo-delete named human-assisted; user attempted full-Demo editor undo-delete on 2026-06-14 but bake load made it infeasible | 2026-06-14 | Agent + User |
| R7 solve correctness + perf | Legacy parity diagnostics; canonical-compute acceptance; replacement staging/validation; in-game review; backend timing; final switched/default checks | Windowed probes and captured in-game review | Legacy metrics remain diagnostic under unchanged `R7_TOLERANCE_V1`; intentional output change passes canonical visual/semantic gates; wall-clock comparison is recorded; explicit legacy remains available | Pass for approved compute-default scope - canonical acceptance, scoped saved-output promotion, compute-default in-game review, backend performance comparison, and final switched/default checks are recorded. Future legacy removal requires a separate explicit comparison/removal protocol. | 2026-06-15 | Agent |
...
- Request to user for next session: no R4 human-visible rerun is needed unless a later code change touches ripple timing/material binding, buoyancy coverage binding, WaterSystem sampling, or settled-body sleep behavior. R6 final automated validation is closed; do not request the same full-Demo editor undo-delete check again. If that editor-stack variant must close later, use a lower-cost fixture or targeted editor harness; visible review remains optional and should be requested explicitly if needed.
...
- Date: 2026-06-15
- Ran by: Agent (R7 probe-only `FRAGCOORD` diagnostic and canonical-compute pivot)
- Godot version/renderer/device: Godot 4.6.3 console; windowed RenderingDevice probe with repo-local `APPDATA`/`LOCALAPPDATA`; headless R6 surface/property guard.
- Command, scene, or workflow: Added a probe-only `FRAGCOORD` pressure-Jacobi shader variant to the controlled legacy pass-6 sampler diagnostic, reran the stack probe, reran the compute backend skeleton regression, reran the R6 public surface/property guard, and ran `git diff --check`.
- Output or parser errors: none in final runs. The known `Demo.tscn` invalid UID fallback warning repeated during surface/property dumping. `git diff --check` passed with only the existing CRLF normalization warning for `session-handoff.md`.
- Visible result, if applicable: n/a for user-facing visuals; this slice remains non-replacing and does not write compute output into generated bake resources.
- Stable result marker: `R7_COMPUTE_SOLVE_FILTER_STACK_OK`; `R7_COMPUTE_BACKEND_SKELETON_OK`; `R7_R6_SURFACE_PROPERTY_DIFF_OK files=10 baseline=post-r6-final current=compute-solve-stack-fragcoord-diagnostic surface_line_numbers=normalized`; `git diff --check`.
- Pass/partial/fail: Historical partial pass for that diagnostic slice - the `FRAGCOORD` diagnostic supports the UV-artifact hypothesis by collapsing legacy y-band X transitions from 20 to 0. Later R7 validation supersedes the then-unrun replacement state with accepted compute-default evidence.
- Notes or follow-up: Compute pressure feedback is now the canonical solver target. Legacy CanvasItem parity remains compatibility evidence and fallback behavior; future replacement requires either legacy `R7_TOLERANCE_V1` parity or `R7_COMPUTE_CANONICAL_ACCEPTANCE_V1` with visual/semantic/fallback/signature evidence.

Recorded result:

- Date: 2026-06-14
- Ran by: Agent (R7 non-replacing pressure-Jacobi stack proof)
- Godot version/renderer/device: Godot 4.6.3 console; windowed RenderingDevice probe with repo-local `APPDATA`/`LOCALAPPDATA`; headless R6 surface/property guard.
- Command, scene, or workflow: Added a report-only production-shaped pressure-Jacobi stack behind `RiverFlowmapBaker`, using RGBA32F ping-pong pressure storage textures, the real stride/iteration schedule, intra-list compute barriers, one submit, delayed wait/sync/readback, and a legacy shader multi-pass intermediate comparison. Re-ran the R6 public surface/property guard and the existing R7 compute backend skeleton probe.
- Output or parser errors: none in final runs. The known `Demo.tscn` invalid UID fallback warning repeated during surface/property dumping.
- Visible result, if applicable: n/a for user-facing visuals; this slice remains non-replacing and does not write compute output into generated bake resources.
- Stable result marker: `R7_COMPUTE_SOLVE_FILTER_STACK_OK`; `R7_R6_SURFACE_PROPERTY_DIFF_OK files=10 baseline=post-r6-final current=r7-compute-solve-stack surface_line_numbers=normalized`; skeleton regression `R7_COMPUTE_BACKEND_SKELETON_OK`; `git diff --check`.
- Pass/partial/fail: Historical partial pass for that diagnostic slice - **R7 pressure-Jacobi stack proof closed**. Later R7 validation supersedes the then-unrun generated-output replacement/performance state with accepted compute-default evidence.
- Notes or follow-up: The stack uses the pressure-intermediate gate `R7_PRESSURE_JACOBI_STACK_INTERMEDIATE_V1`, which does not relax final generated texture `R7_TOLERANCE_V1`. Superseded by the later expanded projection, sampler, pass-prefix, and `FRAGCOORD` diagnostics; next R7 work is `R7_COMPUTE_CANONICAL_ACCEPTANCE_V1`.

Recorded result:

- Date: 2026-06-14
- Ran by: Agent (R7 isolated non-replacing solve/filter compute step)
- Godot version/renderer/device: Godot 4.6.3 console; windowed RenderingDevice probe with repo-local `APPDATA`/`LOCALAPPDATA`; headless R6 surface/property guard.
- Command, scene, or workflow: Added a report-only pressure-Jacobi compute step behind `RiverFlowmapBaker`, compared RGBA32F compute output against a deterministic CPU reference, and confirmed the low-cost fixture's RiverManager bake texture/resource state was unchanged before and after the compute run. Re-ran the R6 public surface/property guard and the existing R7 compute backend skeleton probe.
- Output or parser errors: none in final runs. The known `Demo.tscn` invalid UID fallback warning repeated during surface/property dumping.
- Visible result, if applicable: n/a for user-facing visuals; this slice remains non-replacing and does not write compute output into generated bake resources.
- Stable result marker: `R7_COMPUTE_SOLVE_FILTER_STEP_OK`; `R7_R6_SURFACE_PROPERTY_DIFF_OK files=10 baseline=post-r6-final current=r7-compute-solve-filter surface_line_numbers=normalized`; skeleton regression `R7_COMPUTE_BACKEND_SKELETON_OK`; parser check; `git diff --check`.
- Pass/partial/fail: Historical partial pass for that diagnostic slice - **R7 isolated solve/filter compute slice closed**. Later pressure-stack, canonical acceptance, production replacement validation, performance comparison, and switched/default validation supersede the then-unrun replacement/performance state.
- Notes or follow-up: A later pressure-stack proof is recorded above, and later diagnostics expanded through projection and `FRAGCOORD`. Next R7 work is `R7_COMPUTE_CANONICAL_ACCEPTANCE_V1`.

Recorded result:

- Date: 2026-06-14
- Ran by: Agent (R6.5 cleanup/interface tightening, branch `r6`)
- Godot version/renderer/device: Godot 4.6.3 console; headless with repo-local `APPDATA`/`LOCALAPPDATA` profiles under `.codex-research/godot-user-r65-*`
- Command, scene, or workflow: Removed dead private RiverManager helpers `_cleanup_flowmap_baker()` and `_filter_output_is_valid()` after active-reference audit; kept `_flowmap_bake_renderer` as a serialized property-list compatibility slot and corrected its ownership comment. Ran parser/preload, constants shadow, baseline surface/property dump, R5 guard, R4 guard, and whitespace checks.
- Output or parser errors: none in final runs. The known `Demo.tscn` invalid UID fallback warning for `uid://bn0wo1sl68vtd` repeated during the constants shadow probe. Saved `.res` resources were already dirty from the attempted full-Demo editor check, so R6.5 compared only public surface and property-list outputs, not saved-resource dictionaries.
- Visible result, if applicable: n/a for user-facing visuals; cleanup touched private helpers/comments only.
...
- Pass/partial/fail: Pass — **R6.5 cleanup/interface tightening closed**.
- Notes or follow-up: Superseded for R7 by the 2026-06-14 validation-only gates, non-replacing compute backend skeleton, isolated solve/filter compute step, and pressure-Jacobi stack proof; next R7 work is divergence/gradient/tangency compute before full generated-output coverage.
...
- Notes or follow-up: Superseded for R6 by the 2026-06-14 final automated R6 validation and for R7 by the recorded research, baseline/fixture, format/sync, and non-replacing compute skeleton gates.
...
- R4.2: interactive curve editing on a 50-point river. R4.3: per-tick buoyancy sampling. R7: explicit `legacy_canvas_item` versus `canonical_compute_replacing` bake timing on the low-cost fixture, `res://Demo_obstacle_flow_test.tscn`, and `res://Demo.tscn`.
...
- R7: backend performance comparison is recorded by `R7_COMPUTE_BACKEND_PERFORMANCE_COMPARE_OK`; compute is faster in all recorded cases, but full authored scenes still exceed 1000 ms frame gaps in both paths, so full-scene responsiveness remains shared bake-workload follow-up rather than a compute-default blocker.
...
- R6.5-specific disposable artifacts: `.codex-research/r6-baselines/post-r6-r65/` and `.codex-research/godot-user-r65-*`
