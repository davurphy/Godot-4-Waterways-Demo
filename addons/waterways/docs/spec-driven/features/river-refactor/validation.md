...
- R7's compute path stays fenced against stale dispatch/readback ordering, reports legacy `R7_TOLERANCE_V1` parity metrics, and replaces generated bake output only after either legacy parity passes or `R7_COMPUTE_CANONICAL_ACCEPTANCE_V1` passes with visual/semantic/fallback/signature evidence; bake wall-clock improvement is measured and recorded before shipping replacement.

## Current Validation Snapshot

- R7 update (2026-06-15): validation-only gates, the non-replacing compute backend skeleton, the isolated production-shaped pressure-Jacobi solve/filter compute step, the non-replacing production-shaped pressure-Jacobi stack, expanded projection diagnostic, sampler diagnostics, probe-only `FRAGCOORD` diagnostic, and automated canonical-compute acceptance gate behind `RiverFlowmapBaker` are recorded. Current markers: `R7_COMPUTE_CANONICAL_ACCEPTANCE_V1_AUTOMATED_OK`, `R7_COMPUTE_SOLVE_FILTER_STACK_OK`, `R7_R6_SURFACE_PROPERTY_DIFF_OK`, skeleton regression `R7_COMPUTE_BACKEND_SKELETON_OK`, and earlier `R7_COMPUTE_SOLVE_FILTER_STEP_OK`. Generated bake output replacement remains intentionally unrun.

- Overall status: Partial (track) - **Phases R0, RT, R1, R2, R3, R4, R5, R6, and R8 have implementation/automated validation complete**. R6.5 cleanup/interface tightening passed focused validation. R7's compute-first decision, validation-only gates, non-replacing compute backend skeleton, isolated solve/filter compute step, pressure-Jacobi stack proof, expanded projection diagnostic, sampler diagnostics, `FRAGCOORD` architecture pivot, and automated canonical acceptance proof are recorded; generated bake output replacement remains intentionally unrun. The optional R6 full-Demo editor undo-delete check was attempted but infeasible due bake load, with lower-cost editor/visible review still optional.
- Last automated/windowed pass: 2026-06-15 R7 canonical acceptance - Godot 4.6.3 console/windowed + headless surface guard: `R7_COMPUTE_CANONICAL_ACCEPTANCE_V1_AUTOMATED_OK`, `R7_COMPUTE_SOLVE_FILTER_STACK_OK`, `R7_R6_SURFACE_PROPERTY_DIFF_OK files=10 baseline=post-r6-final current=compute-canonical-acceptance surface_line_numbers=normalized`, skeleton regression `R7_COMPUTE_BACKEND_SKELETON_OK`, and `git diff --check`.
...
- Highest-risk unproven behavior: the remaining high-risk unproven work belongs to future R7 replacement/performance closure. R7 compute is still report-only and must not yet write into RiverManager textures/resources. Replacement now requires either legacy `R7_TOLERANCE_V1` parity or `R7_COMPUTE_CANONICAL_ACCEPTANCE_V1` with visual/semantic/fallback/signature evidence. R6 full-Demo editor undo-delete was attempted but infeasible due bake load; any future editor-stack closure needs a lower-cost fixture or targeted harness, and visible review should not be claimed by automated probes.
...
| --- | --- | --- | --- | --- | --- | --- |
| R7 current compute slice | `r7_compute_solve_filter_stack_probe.gd`; R6 surface/property guard; skeleton regression | Windowed RenderingDevice probe + headless surface guard | Production-shaped pressure/projection diagnostics stay non-replacing, report legacy parity metrics, and leave RiverManager texture/resource state unchanged | Partial pass - `R7_COMPUTE_CANONICAL_ACCEPTANCE_V1_AUTOMATED_OK`; `R7_COMPUTE_SOLVE_FILTER_STACK_OK`; `R7_R6_SURFACE_PROPERTY_DIFF_OK files=10 baseline=post-r6-final current=compute-canonical-acceptance surface_line_numbers=normalized`; skeleton `R7_COMPUTE_BACKEND_SKELETON_OK`; earlier one-step `R7_COMPUTE_SOLVE_FILTER_STEP_OK`; visual review/replacement readiness still pending | 2026-06-15 | Agent |
...
| R7 solve correctness + perf | Legacy parity diagnostics; canonical-compute acceptance; adversarial ordering test; wall-clock before/after | Editor/windowed probe | Legacy metrics reported; intentional output change passes `R7_COMPUTE_CANONICAL_ACCEPTANCE_V1`; fence prevents stale read when dependent dispatches run; wall-clock recorded as regression budget | Partial - non-replacing one-step, pressure-stack, expanded projection, sampler, `FRAGCOORD`, and automated canonical acceptance diagnostics passed for report-only use; full visual acceptance, generated-output replacement, and performance closure remain unrun | 2026-06-15 | Agent |
...
- Pass/partial/fail: Partial pass - the `FRAGCOORD` diagnostic supports the UV-artifact hypothesis by collapsing legacy y-band X transitions from 20 to 0, but generated bake output replacement remains unrun by design.
...
- Pass/partial/fail: Partial pass - **R7 pressure-Jacobi stack proof closed**. Full generated-output replacement and bake performance closure remain unrun by design.
...
- Pass/partial/fail: Partial pass - **R7 isolated solve/filter compute slice closed**. Full generated-output replacement and bake performance closure remain unrun by design.
