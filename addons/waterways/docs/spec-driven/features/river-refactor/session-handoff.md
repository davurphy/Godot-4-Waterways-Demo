...
- Known failing or unproven check: both demo scenes reference the **same** `WaterSystem.water_system_bake.res`, and regeneration saves in place (`water_helper_methods.gd:60-69`) — so only one scene's map can be fresh at a time. The user's regeneration from the obstacle scene (commit `fed4967a`) made the obstacle scene fresh and flipped Demo.tscn to stale. Fix: clear `bake_data` on the obstacle scene's WaterSystem, regenerate (new scene-owned resource), save; then regenerate Demo's. Pre-R2 baseline on a fresh map: influence-zone angular p90 27.1°/27.6° vs the 20° gate and a 12.5° control floor (the Defect-1 signature RT.3 exists to catch — now proven against freshly generated data, not stale data).
...
- [ ] (User, editor) Un-share the system bake: in `Demo_obstacle_flow_test.tscn`, clear the WaterSystem's `bake_data` in the Inspector, Generate System Map (saves a new scene-owned `.res`), save the scene; then open `Demo.tscn`, Generate System Map (restores the shared path as Demo-owned), save. The user's first regeneration (commit `fed4967a`) saved in place into the shared resource, which flipped the staleness to Demo.tscn — both scenes can't be fresh while sharing one file. Verify afterwards with the RT.3 report-mode command (expect no `SYSTEM_FLOW_COMPARE_STALE` and exit 0).
