...
- Status: Accepted and in execution — Phases R0, RT, R1, R2, R3, R4, R5, and R8 complete and fully closed. R5 structural dedup landed with RT.1 scratch rebake texture hashes, exact property-list diff, and `r5_behavior_preservation_probe.gd` passing; R6/R7 are gated on their own docs
...
- Last meaningful decision: R5 keeps `set_from_bake()` as a compatibility wrapper but moves active bake writes to field assignment plus `RiverBakeData.finalize()`; whole `.res` ResourceSaver hashes are not the R5 gate, per-texture RT.1 hashes are. Prior: R4 separates impulse render, simulation step, and impulse clear across frames after the first visible review proved the same-frame ordering could hide all ripples (2026-06-13), R8 restored doc-code coherence (2026-06-12), and R2's gate was restructured on measured evidence.
