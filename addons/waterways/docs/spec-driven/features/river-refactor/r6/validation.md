...
- Overall status: R6.5 cleanup/interface tightening is complete after final automated R6 validation. R6 documentation is tracked, pre-move bake/source evidence exists, R6.3/R6.4/R6.1B-H/R6.2 slices have focused validation, final full-R6 generated-output plus canonical dictionary/API/signal/property gates pass, and the R6.5 private-method cleanup preserved public surface/property shape.
- Last automated pass: 2026-06-14 R6.5 focused validation. Passed `R6_R61F_PARSER_OK`; passed `R6_R62_CONSTANTS_SHADOW_OK comparisons=8 rows=109 signature_version=28`; passed `R6_BASELINE_DUMP_OK out=res://.codex-research/r6-baselines/post-r6-r65 files=10`; passed `R6_R65_SURFACE_PROPERTY_DIFF_OK files=4 baseline=post-r6-final current=post-r6-r65 surface_line_numbers=normalized`; passed `R5_BEHAVIOR_PRESERVATION_PROBE_OK`, `R4_RUNTIME_ROBUSTNESS_PROBE_OK`, and `git diff --check`. The known `Demo.tscn` invalid UID warning repeated during scene-loading probes.
- Last human-assisted attempt: 2026-06-14 R6 editor undo-delete on full `res://Demo.tscn` was attempted by the user, but the bake saturated CPU/GPU enough that the editor lag prevented deleting `WaterSystem/Water River` during the awaited phase. Treat this as an infeasible manual check on the full demo scene, not as a behavior failure.
- Highest-risk unproven behavior: human-visible editor undo-delete on a lower-cost fixture and visible editor/menu review. Source timing remains a watch item for future changes, but final automated R6 before/after parity, R6.1H helper-internal await abort coverage, and the R6.2 live dictionary switch are now closed.
...
| Canonical dictionary dumps are stable | `river_surface_snapshot_probe.gd` | Console/headless | Sorted dumps generated for both demo river bakes | Pass - `R6_BASELINE_DUMP_OK`; six canonical dictionary dumps written under `.codex-research/r6-baselines/pre-r6/`, filtering only `source_metadata.bake_revision` | 2026-06-13 | Agent |
| Constants-table live builder matches old dictionaries | `r6_constants_shadow_probe.gd`; `river_surface_snapshot_probe.gd` plus canonical file compare | Console/headless | Exact saved `source_signature`, exact saved `bake_settings`, saved `source_metadata` match except `bake_revision`, live scene source signatures match, and post-switch canonical dictionary dumps match the post-R6.1A-G rerun baseline | Pass - `R6_R62_CONSTANTS_SHADOW_OK comparisons=8 rows=109 signature_version=28`; `R6_BASELINE_DUMP_OK out=res://.codex-research/r6-baselines/post-r6-r62-live files=10`; `R6_R62_LIVE_CANONICAL_DICTIONARY_DIFF_OK files=6 baseline=post-r6-r61ag-rerun current=post-r6-r62-live metadata_filter=bake_revision`; follow-up `git diff --check` passed | 2026-06-14 | Agent |
| Source signature unchanged | Canonical dump diff before/after R6 | Console/headless | Empty diff, no ignored keys | Pass final - `R6_FINAL_CANONICAL_SURFACE_DIFF_OK files=10 baseline=pre-r6 current=post-r6-final surface_line_numbers=normalized metadata_filter=bake_revision`; signature dumps matched exactly with no ignored keys | 2026-06-14 | Agent |
| Bake settings unchanged | Canonical dump diff before/after R6 | Console/headless | Empty diff, no ignored keys | Pass final - `R6_FINAL_CANONICAL_SURFACE_DIFF_OK files=10 baseline=pre-r6 current=post-r6-final surface_line_numbers=normalized metadata_filter=bake_revision`; bake settings dumps matched exactly with no ignored keys | 2026-06-14 | Agent |
| Source metadata unchanged except dynamic allow-list | Canonical dump diff before/after R6 | Console/headless | Empty diff after ignoring only `bake_revision` | Pass final - `R6_FINAL_CANONICAL_SURFACE_DIFF_OK files=10 baseline=pre-r6 current=post-r6-final surface_line_numbers=normalized metadata_filter=bake_revision`; metadata matched after the existing `bake_revision` filter only | 2026-06-14 | Agent |
| Generated texture output unchanged | RT.1 scratch rebake hash compare for Demo and obstacle demo river bakes | Console/windowed as needed | All generated texture hashes identical | Pass final - `R6_GENERATED_TEXTURE_HASH_OK out=res://.codex-research/r6-baselines/post-r6-r62-live-generated files=2`; `R6_FINAL_GENERATED_TEXTURE_DIFF_OK files=2 baseline=post-r6-r61ag-rerun-generated current=post-r6-r62-live-generated`; `R6_FINAL_RT1_GENERATED_TEXTURE_DIFF_OK files=2 baseline=pre-r6-r61e-generated current=post-r6-r62-live-generated` | 2026-06-14 | Agent |
| Intermediate source images unchanged | `river_source_image_hash_probe.gd` for the full raw-plus-margin list | Console/headless or windowed as needed | All intermediate hashes identical before/after source-helper moves | Pass for R6.1A-G rerun after R6.1H with the existing obstacle caveat - Demo matched the post-R6.1D final source-image dump exactly; obstacle differed only in the four previously documented collision-derived rows and matched the old-code fresh replay exactly: `R6_R61D_COLLISION_VARIANT_RECONCILED_OK ignored_collision_probe_variants=4 baseline=pre-r6-r61d-fresh-rerun current=post-r6-r61ag-rerun-source` | 2026-06-14 | Agent |
| Public RiverManager API and signals unchanged | `river_surface_snapshot_probe.gd` public method/signal surface dump and diff; archived pre-R6 progress capture | Console/headless plus windowed console for progress | Empty diff; `river_changed` and `progress_notified` still present; progress order remains documented | Pass final - `R6_FINAL_CANONICAL_SURFACE_DIFF_OK files=10 baseline=pre-r6 current=post-r6-final surface_line_numbers=normalized metadata_filter=bake_revision`; generated-texture logs still preserve Jacobi labels `Projecting flow 0/40 (stride 32)` through `35/40` | 2026-06-14 | Agent |
| Full inspector property list unchanged | `river_surface_snapshot_probe.gd` serialized property-list dump and diff | Console/windowed as needed | Empty diff | Pass final - full Demo and obstacle RiverManager property-list dumps match the original pre-R6 baseline exactly; only public method/signal source line numbers were normalized | 2026-06-14 | Agent |
| Source timing contract preserved or deliberately validated | Archived flow-speed trap evidence plus future before/after checks | Console/windowed/human | Reports targeted stage and old/new value use, with `flow_speeds` minimum trap case | Baseline captured - `R6_MID_BAKE_TIMING_OK`; mutation at first `Projecting flow` progress left all trap textures identical to same-run neutral control while final metadata/signature re-read mutated `flow_speeds` | 2026-06-13 | Agent |
...
| Bake abort cleanup is complete | `r6_abort_matrix_probe.gd` plus named human-assisted cases | Console/headless plus named editor/human | No stuck flag, leaked renderer, orphan viewport, stale progress source, or partial overwrite | Pass for R6.1H - `R6_R61H_ABORT_MATRIX_OK` covers pre-renderer abort before renderer setup, scene close before renderer setup, terrain-contact helper node-free, renderer-live/Jacobi-labelled cancellation, forced invalid filter output, filter-renderer setup failure, duplicate RiverManager and baker requests, repeated `abort()`, success cleanup, post-renderer/image-postprocess synchronous strategy, and synchronous RiverManager result-application strategy. Editor undo-delete was attempted manually on the full Demo scene but was infeasible due CPU/GPU saturation; use a lower-cost fixture if this editor-stack variant needs future closure. Forced collision-helper null injection is named until a narrow injection seam exists. | 2026-06-14 | Agent + User |
| Existing R5 behavior remains green | `r5_behavior_preservation_probe.gd` | Console/headless | `R5_BEHAVIOR_PRESERVATION_PROBE_OK` | Pass final - expected width-padding warnings emitted | 2026-06-14 | Agent |
| Existing runtime robustness remains green | `r4_runtime_robustness_probe.gd` | Console/headless | `R4_RUNTIME_ROBUSTNESS_PROBE_OK` | Pass final - stable marker reported | 2026-06-14 | Agent |
| Filter renderer load remains green | `filter_renderer_load_check.gd` or equivalent | Console/headless | `FILTER_RENDERER_LOAD_OK shader_paths=19` | Pass final - scratch `.codex-research/filter_renderer_load_check.gd` reported `shader_paths=19` | 2026-06-14 | Agent |
| System-flow mechanism remains green | `system_flow_projected_gate_probe.gd` | Windowed console | `SYSTEM_FLOW_PROJECTED_GATE_OK` | Pass final - Demo report-only scene had `repeat_diff_texels=0` and `slide_diff_texels=0`; obstacle scene had `repeat_diff_texels=0`, `slide_diff_texels=4641`, `slide_mean_angle=3.32347963823468`, and `slide_max_angle=42.2737449054068` | 2026-06-14 | Agent |
...
| Parent docs reflect R6 result only after validation | Parent `validation.md`, `tasks.md`, `roadmap.md`, handoff docs | Docs review | R6 row added only after validation runs | Pass - parent roadmap/tasks/validation/session-handoff and latest handoff now point to final R6 automated validation and the true R6.5/R7 next work | 2026-06-14 | Agent |
| R6.5 cleanup/interface tightening preserves surface | Dead-private-method audit; `river_surface_snapshot_probe.gd`; R5/R4 guards; `git diff --check` | Static + console/headless | Unreferenced private helpers removed, no public API/signal/property-list drift, no signature bump | Pass - removed only `_cleanup_flowmap_baker()` and `_filter_output_is_valid()`; public method/signal dump matched after line-number normalization, property lists matched byte-for-byte, parser/preload plus R5/R4 guards passed | 2026-06-14 | Agent |
...
- Probe/script callers in active source: no active probe directly invokes either wrapper before R6.4; `addons/waterways/probes/river_surface_snapshot_probe.gd` asserts both wrappers remain in the public RiverManager method surface.
...
- Editor undo-delete during an awaited bake is the human-assisted editor-stack version of the automated scene-close/node-free cases. The scripted probe covers the runtime lifecycle effect. The full `res://Demo.tscn` editor attempt was infeasible due bake load, so any future closure of this editor-stack variant should use a lower-cost fixture or targeted editor harness.
...
- Stay in RiverManager: public API and inspector properties, preflight and duplicate bake request guard, mesh generation/material binding, `_write_bake_data()` result application, final dynamic values for `source_metadata`, `get_bake_source_signature()`, and `_get_bake_settings()`, external save attempt, final shader parameter refresh, progress signal ownership, editor property notification, runtime ripple wrappers, and editor validation wrappers. R6.2 moves only the constant dictionary rows to `RiverBakeConstants`.
...
- If this editor-stack variant still needs closure, use a lower-cost fixture or targeted editor harness to start a river bake, undo-delete or remove the river during an awaited phase, then confirm the editor remains usable and a fresh bake can run. Do not ask the user to repeat the full `res://Demo.tscn` version; it was attempted on 2026-06-14 and was too CPU/GPU intensive to perform.
...
- Date: 2026-06-14
- Ran by: Agent
- Godot version/renderer/device: Godot 4.6.3 console; headless for parser/preload, constants shadow, baseline dump, R5 behavior preservation, R4 runtime robustness, and whitespace checks.
- Command, scene, or workflow: Completed R6.5 cleanup/interface tightening. Audited private RiverManager methods for active references, removed the unreferenced `_cleanup_flowmap_baker()` and `_filter_output_is_valid()` helpers, and tightened the `_flowmap_bake_renderer` comment to record property-list compatibility while naming `RiverFlowmapBaker` as the live renderer owner. Generated R6.5 surface/property dumps under `.codex-research/r6-baselines/post-r6-r65/` and compared public method/signal dumps against `.codex-research/r6-baselines/post-r6-final/` with line numbers normalized, plus exact property-list hashes.
- Output or parser errors: No parser/probe failures. The known `Demo.tscn` invalid UID warning repeated during constants and baseline scene loads. `r5_behavior_preservation_probe.gd` emitted its expected width-padding sanitizer warnings. Saved bake resources were already dirty in git status from the manual editor attempt before this validation; R6.5 did not use saved-resource dictionary diffs as its cleanup signal.
- Visible result, if applicable: Headless validation only; no manual editor UI or visual bake review requested for this private cleanup.
- Stable result marker: `R6_R61F_PARSER_OK`; `R6_R62_CONSTANTS_SHADOW_OK comparisons=8 rows=109 signature_version=28`; `R6_BASELINE_DUMP_OK out=res://.codex-research/r6-baselines/post-r6-r65 files=10`; `R6_R65_SURFACE_PROPERTY_DIFF_OK files=4 baseline=post-r6-final current=post-r6-r65 surface_line_numbers=normalized`; `R5_BEHAVIOR_PRESERVATION_PROBE_OK`; `R4_RUNTIME_ROBUSTNESS_PROBE_OK`; `git diff --check`.
- Pass/partial/fail: Pass for R6.5 cleanup/interface tightening. This does not claim the optional full-Demo editor undo-delete residual; that manual workflow remains infeasible on the full scene.
- Notes or follow-up: R6.5 scratch artifacts are disposable: `.codex-research/r6-baselines/post-r6-r65/`, `.codex-research/godot-user-r65-parser/`, `.codex-research/godot-user-r65-constants/`, `.codex-research/godot-user-r65-baseline/`, `.codex-research/godot-user-r65-r5/`, and `.codex-research/godot-user-r65-r4/`.

Recorded result:

- Date: 2026-06-14
- Ran by: Agent
- Godot version/renderer/device: Godot 4.6.3 console; headless for the constants-table shadow probe and canonical dump probe.
- Command, scene, or workflow: Switched only RiverManager's live constant dictionary portions to `RiverBakeConstants`: `_write_bake_data()` now builds `source_metadata` from explicit dynamic values plus table constants, `get_bake_source_signature()` now builds dynamic final-read signature values plus table constants, and `_get_bake_settings()` now builds dynamic settings plus table constants. RiverManager still owns source-generation timing, final dynamic reads, `RiverBakeData` assignment/finalization, resource saving, material binding, `valid_flowmap`, bake flag clearing, completion progress, and result application. The probe used repo-local profiles `.codex-research/godot-user-r62-live` and `.codex-research/godot-user-r62-live-dump`; canonical dumps were written to `.codex-research/r6-baselines/post-r6-r62-live/`.
- Output or parser errors: No parser or probe failures. The known `Demo.tscn` invalid UID warning repeated for `res://waterways_bakes/Demo_28018/WaterSystem.water_system_bake.res`.
- Visible result, if applicable: Headless validation only; no manual editor UI or visual bake review.
- Stable result marker: `R6_R62_CONSTANTS_SHADOW_OK comparisons=8 rows=109 signature_version=28`; `R6_BASELINE_DUMP_OK out=res://.codex-research/r6-baselines/post-r6-r62-live files=10`; `R6_R62_LIVE_CANONICAL_DICTIONARY_DIFF_OK files=6 baseline=post-r6-r61ag-rerun current=post-r6-r62-live metadata_filter=bake_revision`; final `git diff --check`.
- Pass/partial/fail: Pass for the R6.2 live dictionary-generation switch. At the time, this was not a full R6 pass because final full-R6 before/after diffs, system-flow projected gate, and visible/manual editor checks remained open; final automated R6 validation later closed the generated/canonical/system-flow gates.
- Notes or follow-up: Bake signature version remains 28. `source_metadata.bake_revision` is still the only dynamic metadata diff filtered by the canonical dump workflow.

Recorded result:

- Date: 2026-06-14
- Ran by: Agent
- Godot version/renderer/device: Godot 4.6.3 console; headless for the constants-table shadow probe.
- Command, scene, or workflow: Added `addons/waterways/river_bake_constants.gd` with 109 reviewable rows for metadata/signature/settings constant or generated-literal values, explicit dynamic key lists, required row-type validation, and `reason`/`review_decision` checks for non-signature rows. Added `addons/waterways/probes/r6_constants_shadow_probe.gd`, which compares saved Demo and obstacle Demo `source_metadata`, `source_signature`, and `bake_settings` dictionaries plus live scene source signatures against table-generated dictionaries using the R6 canonical dump rules. The probe used repo-local `APPDATA`/`LOCALAPPDATA` under `.codex-research/godot-user-r62-shadow`.
- Output or parser errors: No parser or probe failures. The known `Demo.tscn` invalid UID warning repeated for `res://waterways_bakes/Demo_28018/WaterSystem.water_system_bake.res`.
- Visible result, if applicable: Headless validation only; no manual editor UI or visual bake review.
- Stable result marker: `R6_R62_CONSTANTS_SHADOW_OK comparisons=8 rows=109 signature_version=28`; final `git diff --check`.
- Pass/partial/fail: Pass for the R6.2 shadow constants-table slice. Superseded by the R6.2 live dictionary-generation switch result recorded above.
- Notes or follow-up: The live dictionary path was intentionally not switched in this earlier shadow-only result. Bake signature version remained 28.
...
- Pass/partial/fail: Pass for the R6.1A-G validation rerun after R6.1H. This confirms the earlier bake-extraction coverage did not regress after the abort-matrix work. At the time, this was not a full R6 pass because constants-table extraction, final full-R6 before/after diffs, system-flow projected gate, and visible/manual editor checks remained open; constants and final automated validation later closed.
...
- Pass/partial/fail: Pass for R6.1H abort-matrix coverage. At the time, this was not a full R6 pass because constants-table extraction, final full-R6 before/after diffs, system-flow projected gate, and visible/manual editor checks remained open; constants and final automated validation later closed.
...
- Pass/partial/fail: Pass for the R6.1C run-pass helper and focused invalid-output cleanup slice. At the time, this was not a full R6 pass because source-image movement, full pass sequencing, full abort matrix, fresh scratch rebake comparison, and final before/after gates remained open; later R6 validation closed those automated gates.
...
- Pass/partial/fail: Pass for the R6.1B baker shell and thin renderer lifecycle slice. At the time, this was not a full R6 pass because pass sequencing, source-image movement, full abort matrix, fresh scratch rebake comparison, and final before/after gates remained open; later R6 validation closed those automated gates.
...
- Pass/partial/fail: Pass for the R6.4 editor validation extraction slice. At the time, this was not a full R6 pass because bake extraction, abort matrix, fresh scratch rebake comparison, and final before/after gates remained open; later R6 validation closed those automated gates.
...
- Pass/partial/fail: Pass for the R6.3 runtime ripple extraction slice. At the time, this was not a full R6 pass because bake extraction, editor extraction, fresh scratch rebake comparison, abort matrix, and final before/after gates remained open; later R6 validation closed those automated gates.
...
- Notes or follow-up: Baseline artifacts are in `.codex-research/r6-baselines/pre-r6/`. `source_metadata` dumps filter only `bake_revision`; no `bake_revision` remains in the metadata dump files. At the time, fresh scratch rebakes, source-image hashes, mid-bake timing proof, progress label/order documentation, abort matrix, runtime ripple, editor validation, and final before/after diffs remained open; later R6 validation closed the automated gates.
...
- Date: 2026-06-14
- Ran by: Agent
- Godot version/renderer/device: Godot 4.6.3 console; Vulkan Forward+, AMD Radeon RX 6800 XT for rendered generated-texture and system-flow projected-gate runs; headless for parser/check-only, canonical baseline dump, R5 behavior preservation, R4 runtime robustness, filter-renderer load, generated-output comparison, canonical/surface comparison, and whitespace check.
- Command, scene, or workflow: Ran final R6 validation after the R6.2 live constants-table switch. Fresh generated hashes were written to `.codex-research/r6-baselines/post-r6-r62-live-generated/` for `res://Demo.tscn` and `res://Demo_obstacle_flow_test.tscn`, compared against both `.codex-research/r6-baselines/post-r6-r61ag-rerun-generated/` and the scratch old-code comparator `.codex-research/r6-pre-r61d-origin-fresh/.codex-research/r6-baselines/pre-r6-r61e-generated/`. Fresh canonical dictionary/API/signal/property dumps were written to `.codex-research/r6-baselines/post-r6-final/` and compared against `.codex-research/r6-baselines/pre-r6/`, normalizing only public method/signal source line numbers. Reran R5, R4, filter-renderer load, and the rendered system-flow projected gate, then ran final `git diff --check`.
- Output or parser errors: No parser/check-only errors for `river_manager.gd`, `river_flowmap_baker.gd`, `river_bake_constants.gd`, or `r6_constants_shadow_probe.gd`. The known non-fatal `Demo.tscn` invalid UID warning repeated for `res://waterways_bakes/Demo_28018/WaterSystem.water_system_bake.res`. R5 behavior preservation emitted the expected width-padding warnings.
- Visible result, if applicable: Rendered console validation only; no manual editor UI click review. The system-flow projected gate reported deterministic gated renders and the expected obstacle-scene forced-slide delta.
- Stable result marker: `R6_GENERATED_TEXTURE_HASH_OK out=res://.codex-research/r6-baselines/post-r6-r62-live-generated files=2`; `R6_FINAL_GENERATED_TEXTURE_DIFF_OK files=2 baseline=post-r6-r61ag-rerun-generated current=post-r6-r62-live-generated`; `R6_FINAL_RT1_GENERATED_TEXTURE_DIFF_OK files=2 baseline=pre-r6-r61e-generated current=post-r6-r62-live-generated`; `R6_BASELINE_DUMP_OK out=res://.codex-research/r6-baselines/post-r6-final files=10`; `R6_FINAL_CANONICAL_SURFACE_DIFF_OK files=10 baseline=pre-r6 current=post-r6-final surface_line_numbers=normalized metadata_filter=bake_revision`; `R5_BEHAVIOR_PRESERVATION_PROBE_OK`; `R4_RUNTIME_ROBUSTNESS_PROBE_OK`; `FILTER_RENDERER_LOAD_OK shader_paths=19`; `SYSTEM_FLOW_PROJECTED_GATE_OK`; final `git diff --check`.
- Pass/partial/fail: Pass for final automated R6 validation. Generated texture hashes, source signature, bake settings, source metadata under the existing `bake_revision` filter, public API/signal surface, and full inspector property lists match their R6 comparison contracts. Human-assisted editor undo-delete and visible editor/menu review are not claimed from headless/rendered console probes alone; the later full-Demo editor undo-delete attempt is recorded below as infeasible due bake load.
- Notes or follow-up: No saved-resource changes appeared in git status after the final validation runs. New scratch artifacts live under `.codex-research/r6-baselines/post-r6-r62-live-generated/`, `.codex-research/r6-baselines/post-r6-final/`, and `.codex-research/godot-user-r6-final-*`.

Recorded result:

- Date: 2026-06-14
- Ran by: User
- Godot version/renderer/device: Godot 4.6.3 editor; full `res://Demo.tscn` manual workflow.
- Command, scene, or workflow: Attempted the remaining R6 editor-stack residual: start a river bake, then undo-delete or remove `WaterSystem/Water River` during an awaited bake phase.
- Output or parser errors: Not captured; the check could not progress to the delete action.
- Visible result, if applicable: The bake was too CPU/GPU intensive on the full Demo scene, causing enough editor lag that the user could not delete the river during the awaited phase.
- Stable result marker: none; manual attempt only.
- Pass/partial/fail: Infeasible manual check on the full Demo scene, not a product-behavior failure. Automated lifecycle coverage remains `R6_R61H_ABORT_MATRIX_OK`; the editor-stack undo-delete variant should use a lower-cost fixture or targeted editor harness if future closure is required.
- Notes or follow-up: Do not ask the user to repeat this exact full-Demo manual check. The user attempt may touch saved bake resources or generate `.uid` sidecars; inspect git status before deciding what belongs in an R6 commit.
...
- Files or folders safe to delete now: R6.5-specific `.codex-research/r6-baselines/post-r6-r65/` and `.codex-research/godot-user-r65-*` are disposable because the markers and comparison result are recorded here; older R6 baselines should stay until R6 review/commit packaging is complete.

## Manual Review Checklist

- [x] Acceptance criteria in `spec.md` are satisfied for automated R6 gates and R6.5 cleanup.
- [x] Source timing contract is preserved or deliberate changes are documented and validated.
- [x] Canonical dictionary dumps are implemented before baseline capture.
- [x] No broad RiverManager reach-back exists in the baker.
- [x] Runtime/editor boundary is preserved.
- [x] Generated resources and metadata remain explicit and inspectable.
- [ ] Human-assisted editor/menu/visible checks are recorded when needed.
- [x] Parent docs and latest handoff are updated only after validation runs.
