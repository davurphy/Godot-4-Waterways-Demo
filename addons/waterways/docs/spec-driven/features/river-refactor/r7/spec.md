...
- No removal of the legacy CanvasItem solution before human-visible in-game reviews compare both preserved solutions and record an accepted decision.
...
- `R7_COMPUTE_CANONICAL_ACCEPTANCE_V1` must be satisfied before accepting a compute path that intentionally differs from legacy CanvasItem output. It requires canonical integer texel-space pressure feedback rules, visual validation in named scenes/views, physics/semantic validation, continued legacy parity diagnostics, a fallback/selection flag, unchanged RiverManager ownership, and an explicit bake signature/version decision if generated textures change. The current five low-cost canonical artifacts are accepted as an intentional visible/output change, and low-cost non-replacing cleanup/heartbeat/non-neutral, explicit selection/guarded abort, representative material/debug visual coverage, the named replacement gate, report-only generated-output replacement staging, source signature v29, refreshed production replacement validation/runtime smoke, broader Demo/obstacle fixture coverage, saved-output promotion, and switched/default compute acceptance are recorded. The code default backend is now accepted compute.
- `canonical_compute_replacing` must remain a fallback-to-legacy request until `R7_CANONICAL_COMPUTE_REPLACEMENT_GATE_V1` is ready. That gate requires recorded canonical acceptance, representative visuals, selection/abort, cleanup/responsiveness, and RiverManager surface evidence; accepted generated-output replacement staging; accepted production replacement validation; source signature version 29 or backend mode included in the source signature; and an enabled replacement code path. With all evidence supplied, the gate is now ready and explicit `canonical_compute_replacing` may select compute; broader fixture coverage confirms that scope across the low-cost, Demo, and obstacle Demo scenes. The two scoped Demo river bake resources are intentionally promoted under source signature v29, and `canonical_compute_replacing` is now the accepted non-explicit default while explicit `legacy_canvas_item` remains available.
- Explicit backend performance evidence must be recorded before any legacy-removal decision. Current status: `R7_COMPUTE_BACKEND_PERFORMANCE_COMPARE_OK` records `legacy_canvas_item` versus `canonical_compute_replacing` on the low-cost fixture, `res://Demo_obstacle_flow_test.tscn`, and `res://Demo.tscn`; compute is faster in all recorded cases, no fallback occurs, and full authored scenes still show >1000 ms frame gaps in both paths.
- Before any legacy-path removal, R7 must record a separate explicit comparison/removal decision in named scenes/views. The requested compute-default in-game review is recorded by `R7_COMPUTE_HUMAN_VISIBLE_INGAME_REVIEW_CAPTURE_OK`, and backend performance evidence is recorded by `R7_COMPUTE_BACKEND_PERFORMANCE_COMPARE_OK`; explicit `legacy_canvas_item` remains available. Any removal protocol must compare the accepted compute default and explicit legacy fallback, preserve both paths during review, and record screenshots or reviewer notes plus Output/console text in `validation.md` with the final decision in `review.md`.
