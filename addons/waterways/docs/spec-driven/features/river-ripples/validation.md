...
- Phase 10 editor tooling stays editor-only, starts with non-mutating read-only status UI, keeps preset selectors transient, applies presets through per-property editor undo, saves captured presets only through explicit user-selected editor paths, keeps visual gizmos non-mutating unless a handle edit contract exists, cleans up scratch UI/preview state across lifecycle events, never dirties scenes from read-only/selector/capture/preview-only paths, and keeps live-scene runtime commands disabled until a separate bridge is validated.

## Current Validation Snapshot

Keep this short and current. Older detailed runs belong in "Recorded Results" below.

- Overall status: Standalone feedback spike passed console/static validation and human-visible review; standalone mapping probe passed console and human-visible validation; standalone mesh-footprint boundary-mask probe passed console and human-visible validation; RiverManager-facing material ownership/restore, minimal river shader neutral path, river shader visible-normal path, river shader sampling-budget, shader-cost/performance, demo review-scene smoke, demo review-scene diagnostic, debug parity, reusable field/emitter lifecycle, field/emitter dispatch-performance, post-review-strength-boost terrain-aware demo-backed field/emitter authoring, Mobile/Compatibility renderer checks, Phase 9 named-class parser/editor-cache checks, Phase 9 in-memory preset-resource/API, export grouping, built-in starter presets, warning cleanup, scratch scene/resource reload, Phase 10 read-only inspector skeleton, Phase 10 transient preset Apply controls, Phase 10 capture/save controls, Phase 10 visual field/emitter gizmo helpers, first undo-backed emitter handle slice, undo-backed field `world_bounds` face-handle slice, and editor-only field boundary preview helper passed console/static validation; demo-backed visible normal review passed the current human-visible base-flow/outward-ring gate; demo-backed debug parity, field/emitter visual/debug/control/stop-reload behavior, Add Node visibility, Phase 9 editor authoring surface, Phase 10 read-only inspector lifecycle/dirty-state behavior, Phase 10 Apply active-preset/undo/no-op/boundary/lifecycle behavior, Phase 10 capture/save save-dialog/dirty-state/restart/no-live-link behavior, Phase 10 visual gizmo readability/dirty-state/lifecycle behavior, Phase 10 emitter radius/moving-threshold drag/undo/redo/lifecycle behavior, Phase 10 field `world_bounds` handle behavior, and Phase 10 boundary preview behavior passed human-visible review.
- Last automated pass: Phase 10 inspector/apply/capture-save/gizmo/handle/boundary-preview contract passed on 2026-06-08 with Godot 4.6.3 Forward+ `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`; the latest pass includes active preset status/selector restoration checks, capture/save checks for unsaved scratch presets and explicit `.tres` save paths, visual gizmo checks, boundary preview checks, emitter handle checks, and field `world_bounds` handle checks: field bounds/footprint/route segments, projected `field_boundary_preview=8` outline points for a two-triangle quad, six stable field face handles `2001..2006`, field handle face-center positions, `world_bounds`-only property mapping, opposite-face preservation, no-op field diff skipping, minimum non-flipped size clamping, emitter radius, moving-emitter threshold, tiny moving-threshold pickability guide/grip, emitter route, no preview children, no exported-value mutation while building helpers, moving emitters exposing radius plus moving-threshold handles, non-moving emitters exposing only radius, emitter handle value clamping to `0.001..64.0`, registered `"handles"` gizmo material use after the editor material lookup error, and no runtime editor/save leakage. Parser checks passed for `ripple_gizmo_handle_model.gd`, `ripple_gizmo_geometry.gd`, `ripple_gizmo.gd`, and the Phase 10 probe after the boundary-preview slice; earlier parser checks passed for `ripple_inspector_preset_apply_model.gd`, `ripple_inspector_status.gd`, `ripple_inspector_plugin.gd`, `plugin.gd`, and `ripple_phase10_inspector_probe.gd`. Post-slice regression probes and static scans are recorded below and passed; the demo review regression needed a normal console-renderer run because the first `--headless` attempt used dummy texture storage. Earlier Phase 9 named Add Node registration passed on 2026-06-07 after adding `class_name` and `@icon` to `WaterRippleField` and `WaterRippleEmitter`; user confirmed both are searchable with icons as child nodes in `Demo.tscn` and `ripple_field_emitter_demo_review.tscn`.
- Last human-assisted result: User confirmed on 2026-06-08 that the Phase 10 boundary preview tests all pass. Earlier the user confirmed the Phase 10 field `world_bounds` handle tests pass cleanly in the editor. Earlier still, the user confirmed the Phase 10 emitter handle review: `PulseEmitter_UpperWater` radius handle changed only `radius` and undo/redo worked; after the tiny-grip fix, `MovingEmitter_TestTrail` moving-threshold grip changed only `moving_emit_distance` and undo/redo worked; plugin disable/re-enable, close/reopen without saving, delete/undo-delete, and run/stop cleanup worked; before/after values and undo/redo behavior worked; Output showed no errors.
- Highest-risk unproven behavior: Response/refraction/displacement tuning remains separate and unproven. Phase 10 emitter handle no-op/cancel behavior remains unchecked because Godot did not expose an obvious gesture in review, and dirty-prompt reporting remains unclear; neither is treated as a blocker for the current handle slice. `debug_visible` re-exposure and live-scene commands remain unimplemented and must follow `phase10-editor-polish-plan.md` one mutation category at a time.
...
| Phase 10 capture/save | `ripple_inspector_plugin.gd` and `ripple_phase10_inspector_probe.gd` | Godot 4.6.3 Forward+ console parser/probe plus local static scan; Godot 4.6.3 editor human-visible review | Capture creates scratch resources only; Save writes only a user-selected preset asset path; runtime scripts/preset resources gain no `ResourceSaver`, `.tres` write path, editor-only class reference, or live link | Pass: automated contract passed in `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`; probe validates unsaved scratch field/emitter captures, no node mutation, saved scratch `.tres` reload with correct scripts/values, no target routing or reserved value persistence, `EditorFileDialog`/`ResourceSaver.save()` isolated to the inspector plugin, and runtime scripts/preset resources free of editor/save leakage. User completed the visible review and confirmed capture alone stayed non-dirty, save/reload survived restart/reopen with correct preset types/values, no node gained a preset slot/path/live link/auto-apply behavior, and Output showed no errors. | 2026-06-08 | Agent/User |
| Phase 10 visual gizmo helpers | `ripple_gizmo.gd`, `ripple_gizmo_geometry.gd`, `plugin.gd`, and `ripple_phase10_inspector_probe.gd` | Godot 4.6.3 Forward+ console parser/probe plus local static scan; Godot 4.6.3 editor human-visible review | Field/emitter gizmos are registered from the editor plugin, draw non-mutating helper lines, do not call runtime rebuild/reset/emission/material/boundary/private APIs, do not leak editor-only classes into runtime scripts, are readable in the editor viewport, do not dirty the scene by viewing, and leave no stale helpers or Output errors through selection/lifecycle/play-stop checks | Pass: `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` validates plugin registration/removal, field bounds `24`, field footprint `12`, explicit target route `2`, emitter radius `128`, moving threshold `64`, emitter route `2`, no preview children, no exported-value mutation while building helpers, forbidden-call boundaries, and no runtime editor/save leakage. Parser checks passed for the gizmo scripts, plugin, and probe; regression probes stayed green. User confirmed the visible editor review before boundary preview was added: field box/footprint/route and emitter radius/moving/route helpers were readable, viewing only did not prompt for unsaved changes, and Output showed no errors. | 2026-06-08 | Agent/User |
| Phase 10 field boundary preview helper | `ripple_gizmo.gd`, `ripple_gizmo_geometry.gd`, and `ripple_phase10_inspector_probe.gd` | Godot 4.6.3 Forward+ console parser/probe plus local static scan; human-visible editor review | Field selection draws an editor-only projected mesh-footprint outline from configured boundary/target mesh sources without generating runtime boundary textures, calling runtime/private target caches, creating preview nodes, mutating exported values, dirtying scenes, changing bakes/materials/WaterSystem/source signatures, or leaking editor-only classes into runtime scripts | Pass: `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` reports `field_gizmo_segments={bounds=24, footprint=12, boundary_preview=8, routes=2}` and validates no preview/runtime children, no exported-value mutation, forbidden-call boundaries, and runtime editor/save leakage absence. Parser checks for `ripple_gizmo_geometry.gd`, `ripple_gizmo.gd`, and the Phase 10 probe passed. Static forbidden-call and runtime leak scans passed. Regressions `RIPPLE_PHASE9_PRESET_API_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PROBE_OK`, `RIPPLE_MATERIAL_OWNERSHIP_PROBE_OK`, and `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_PROBE_OK` passed. User reported the boundary preview tests all pass. | 2026-06-08 | Agent/User |
| Phase 10 emitter gizmo handles | `ripple_gizmo.gd`, `ripple_gizmo_geometry.gd`, `ripple_gizmo_handle_model.gd`, `plugin.gd`, and `ripple_phase10_inspector_probe.gd` | Godot 4.6.3 Forward+ console parser/probe plus local static scan; human-visible drag/undo/redo/lifecycle review | Emitter radius and moving-threshold handles edit only `radius` and `moving_emit_distance` through editor undo, clamp to `0.001..64.0`, skip no-op diffs, restore on cancel when supported, expose a minimum-distance pickable grip/guide for tiny moving thresholds, add no preview children, call no runtime rebuild/reset/emission/material/boundary/private APIs, and leak no editor-only classes into runtime scripts | Human-visible pass for practical behavior: user confirmed radius drag changed only `radius` with undo/redo, confirmed the tiny-grip moving-threshold retry worked for `moving_emit_distance` with undo/redo, and confirmed plugin disable/re-enable, close/reopen without saving, delete/undo-delete, run/stop cleanup, and no Output errors. Godot did not appear to expose an obvious no-op/cancel gesture and that was not checked; dirty-prompt result was unclear. Automated pass: `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` reports `emitter_gizmo_handles={moving_handles=2, pulse_handles=1, tiny_moving_handles=2}` and `emitter_gizmo_segments={tiny_handle_guides=2}`. | 2026-06-08 | Agent/User |
| Phase 10 field bounds gizmo handles | `ripple_gizmo.gd`, `ripple_gizmo_geometry.gd`, `ripple_gizmo_handle_model.gd`, `plugin.gd`, and `ripple_phase10_inspector_probe.gd` | Godot 4.6.3 Forward+ console parser/probe plus local static scan; Godot editor human-visible review | Six field face handles edit only `world_bounds` through editor undo, preserve the opposite AABB face, clamp before flipping below `0.001`, skip no-op diffs, add no preview children, call no runtime rebuild/reset/emission/material/boundary/private APIs from editor helper code, and leak no editor-only classes into runtime scripts | Pass: `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` reports `field_gizmo_handles={field_handles=6, first_id=2001, last_id=2006}` and `field_handle_model={field_property=world_bounds, field_ids=6}`. Parser checks passed for `ripple_gizmo_handle_model.gd`, `ripple_gizmo_geometry.gd`, `ripple_gizmo.gd`, and the Phase 10 probe. Post-slice regression probes, forbidden editor-call scan, runtime editor/save leak scan, and `git diff --check` passed. User confirmed the editor tests pass cleanly. | 2026-06-08 | Agent/User |
...
- Date: 2026-06-08
- Ran by: User
- Godot version/renderer/device: Godot 4.6.3 editor; renderer/device not relayed.
- Command, scene, or workflow: Phase 10 boundary preview human-visible editor review using the updated gizmo review checklist for `res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_field_emitter_demo_review.tscn`.
- Output or parser errors: User reported the tests all pass.
- Visible result, if applicable: Boundary preview behavior accepted by user review.
- Stable result marker: Human-visible pass for Phase 10 boundary preview helper.
- Pass/partial/fail: Pass for the Phase 10 boundary preview editor gate.
- Notes or follow-up: Next optional Phase 10 work is `debug_visible` re-exposure design or live-scene bridge planning; response/refraction/displacement tuning remains a separate plan.

Recorded result:

- Date: 2026-06-08
- Ran by: Agent
- Godot version/renderer/device: Godot 4.6.3 stable, Vulkan Forward+, AMD Radeon RX 6800 XT for the console runs.
- Command, scene, or workflow: Added the editor-only field boundary preview helper; ran profile-isolated parser checks for `ripple_gizmo_geometry.gd`, `ripple_gizmo.gd`, and `ripple_phase10_inspector_probe.gd`; ran `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`; ran regression probes `RIPPLE_PHASE9_PRESET_API_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PROBE_OK`, `RIPPLE_MATERIAL_OWNERSHIP_PROBE_OK`, and `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_PROBE_OK`; ran `git diff --check`; scanned editor helper scripts for forbidden runtime calls and runtime field/emitter scripts plus preset resources for editor/save leakage.
- Output or parser errors: Parser checks exited cleanly. `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` reported `field_gizmo_segments={bounds=24, footprint=12, boundary_preview=8, routes=2}`. The regression markers all passed. Static scans found no forbidden editor helper calls and no runtime editor/save leakage.
- Visible result, if applicable: Not human-visible. This proves the automated contract for drawing a projected mesh-footprint boundary outline without runtime boundary generation, preview children, exported-value mutation, runtime material calls, bakes, WaterSystem/source-signature/buoyancy changes, or editor-only runtime leakage.
- Stable result marker: `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`, `RIPPLE_PHASE9_PRESET_API_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PROBE_OK`, `RIPPLE_MATERIAL_OWNERSHIP_PROBE_OK`, `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_PROBE_OK`
- Pass/partial/fail: Pass for automated Phase 10 boundary preview contract. A later user-run review recorded below closes the human-visible editor readability/lifecycle gate for this slice.
- Notes or follow-up: Boundary preview is accepted for the current Phase 10 gizmo contract. Next choices are `debug_visible` re-exposure, live-scene bridge planning, or a separate response/refraction/displacement tuning plan.
...
- Ran by: User
- Godot version/renderer/device: Godot 4.6.3 editor; renderer/device not relayed.
- Command, scene, or workflow: Phase 10 field `world_bounds` handle human-visible editor review for the six face handles in `res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_field_emitter_demo_review.tscn`.
- Output or parser errors: User reported the tests all pass cleanly in the editor.
- Visible result, if applicable: Field `world_bounds` handle behavior accepted by user review.
- Stable result marker: Human-visible pass for Phase 10 field `world_bounds` handles.
- Pass/partial/fail: Pass for the Phase 10 field bounds handle editor gate.
- Notes or follow-up: Boundary preview automated validation has since passed and now needs human-visible review. `debug_visible` re-exposure and live-scene bridge work remain deferred.

Recorded result:

- Date: 2026-06-08
- Ran by: Agent
- Godot version/renderer/device: Godot 4.6.3 stable; Forward+ for the Phase 10 and demo review console runs; AMD Radeon RX 6800 XT reported by the demo review regression.
- Command, scene, or workflow: Final post-field-handle regression/static sweep. Ran `RIPPLE_PHASE9_PRESET_API_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PROBE_OK`, `RIPPLE_MATERIAL_OWNERSHIP_PROBE_OK`, and `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_PROBE_OK`; ran `git diff --check`; scanned editor helper scripts for forbidden runtime calls; scanned runtime field/emitter scripts and preset resources for editor-only classes, `ResourceSaver`, or `.tres` save-path references.
- Output or parser errors: Regression pass markers were stable. `git diff --check` exited cleanly with only the pre-existing line-ending warning on `addons/zylann.hterrain/util/quad_tree_lod.gd`. Static scans reported `FORBIDDEN_EDITOR_CALL_SCAN_OK` and `RUNTIME_EDITOR_SAVE_LEAK_SCAN_OK`.
- Visible result, if applicable: Not human-visible. The demo review regression first failed under `--headless` because dummy texture storage made texture readback unavailable; the same probe passed when rerun through the normal console renderer.
- Stable result marker: `RIPPLE_PHASE9_PRESET_API_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PROBE_OK`, `RIPPLE_MATERIAL_OWNERSHIP_PROBE_OK`, `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_PROBE_OK`
- Pass/partial/fail: Pass for the post-slice automated regression/static sweep. At that time, this was automated-only; user review later passed.
- Notes or follow-up: Keep treating `--headless` texture readback failures in demo-render probes as an environment launch issue unless a normal renderer rerun also fails.

Recorded result:

- Date: 2026-06-08
- Ran by: Agent
- Godot version/renderer/device: Godot 4.6.3 stable, Vulkan Forward+, renderer device not queried for this headless parser/probe run.
- Command, scene, or workflow: Added undo-backed field `world_bounds` face handles; ran profile-isolated parser checks for `ripple_gizmo_handle_model.gd`, `ripple_gizmo_geometry.gd`, `ripple_gizmo.gd`, and `ripple_phase10_inspector_probe.gd`; ran profile-isolated console probe `--script res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_phase10_inspector_probe.gd`.
- Output or parser errors: Parser checks exited cleanly. Stable marker `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`. The probe reported `field_gizmo_handles={field_handles=6, first_id=2001, last_id=2006}`, `field_handle_model={field_property=world_bounds, field_ids=6, clamped_min_size=0.00099992752075}`, `emitter_gizmo_handles={moving_handles=2, pulse_handles=1, tiny_moving_handles=2}`, and `emitter_gizmo_segments={tiny_handle_guides=2}`.
- Visible result, if applicable: Not human-visible; this validates the automated contract for field face IDs, face-center handle geometry, `world_bounds`-only property mapping, opposite-face preservation, no-op diff skipping, min-size clamping, no preview children, forbidden-call boundaries, and no runtime editor/save leakage.
- Stable result marker: `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`
- Pass/partial/fail: Pass for automated Phase 10 field `world_bounds` handle contract. At that time, this was automated-only; user review later passed.
- Notes or follow-up: Boundary preview automated validation has since passed and now needs human-visible review. `debug_visible` re-exposure and live-scene bridge work remain deferred. The field handle slice edits only the exported `world_bounds` AABB through editor undo and keeps runtime field/emitter scripts free of editor-only references.
...
- Ran by: User
- Godot version/renderer/device: Godot 4.6.3 editor; renderer/device not relayed.
- Command, scene, or workflow: Phase 10 emitter handle human review in `res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_field_emitter_demo_review.tscn`.
- Output or parser errors: User reported no Output errors.
- Visible result, if applicable: `PulseEmitter_UpperWater` radius handle changed only `radius` and undo/redo worked. `MovingEmitter_TestTrail` moving-threshold grip changed only `moving_emit_distance` and undo/redo worked after the tiny-grip fix. Plugin disable/re-enable worked. Close/reopen without saving worked. Delete/undo-delete worked. Run/stop cleanup worked. Before/after values and undo/redo behavior worked.
- Stable result marker: Human-visible pass for practical emitter handle behavior.
- Pass/partial/fail: Pass for practical Phase 10 emitter handle drag/undo/redo and lifecycle behavior. No-op/cancel was not checked because Godot did not appear to expose an obvious gesture; dirty-prompt result was unclear. Neither is treated as a blocker.
- Notes or follow-up: At that time, field `world_bounds` handles were still a future slice. Response/refraction/displacement tuning, boundary previews, `debug_visible` re-exposure, and live-scene bridge work remain separate future slices.

Recorded result:

- Date: 2026-06-08
- Ran by: Agent after user reported the moving-threshold handle was not selectable
- Godot version/renderer/device: Godot 4.6.3 stable, Vulkan Forward+, AMD Radeon RX 6800 XT
- Command, scene, or workflow: User confirmed `PulseEmitter_UpperWater` radius handle drag changed only `radius` and undo/redo worked. User could not select/drag the `MovingEmitter_TestTrail` dashed-ring handle. Root cause: the review scene deliberately uses `moving_emit_distance=0.08` for a dense trail, so the dashed ring and handle were too close to the emitter origin to pick comfortably. Added a minimum-distance pickable grip and short guide line for tiny moving-threshold values, while preserving the actual dashed ring and subtracting the visual offset during drag. Ran profile-isolated parser checks for `ripple_gizmo_handle_model.gd`, `ripple_gizmo_geometry.gd`, and `ripple_gizmo.gd`; ran profile-isolated Phase 10 console probe with `APPDATA`/`LOCALAPPDATA` redirected under `.codex-research`.
- Output or parser errors: Parser checks exited cleanly. `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` passed and reported `emitter_gizmo_segments={radius=128, moving=64, tiny_handle_guides=2, route=2}` plus `emitter_gizmo_handles={moving_handles=2, pulse_handles=1, tiny_moving_handles=2}`.
- Visible result, if applicable: Radius handle human-visible review passed. After this pickability fix, user confirmed the moving-threshold handle works too.
- Stable result marker: `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`
- Pass/partial/fail: Historical partial pass for the Phase 10 emitter handles: core radius and moving-threshold drag/undo/redo behavior passed human-visible review at that time. Later practical review above also passed plugin disable/re-enable, close/reopen, delete/undo-delete, run/stop cleanup, and no Output errors; no-op/cancel remained unchecked and dirty-prompt reporting was unclear.
- Notes or follow-up: For `MovingEmitter_TestTrail`, the true tiny dashed ring remains near the emitter and a short yellow guide points to a more pickable grip. At that time, field `world_bounds` handles were still a future slice.

Recorded result:

- Date: 2026-06-08
- Ran by: Agent after user-visible editor error report
- Godot version/renderer/device: Godot 4.6.3 stable, Vulkan Forward+, AMD Radeon RX 6800 XT
- Command, scene, or workflow: User reported `ERROR: (6) editor/scene/3d/node_3d_editor_gizmos.cpp:1020 - Condition "!materials.has(p_name)" is true. Returning: Ref<StandardMaterial3D>()` while reviewing the new emitter handles. Changed the ripple gizmo handle material from the custom name `"ripple_handles"` to Godot's registered `"handles"` material name, matching the existing river gizmo pattern, and added a Phase 10 probe guard for the material name. Ran profile-isolated parser check for `ripple_gizmo.gd` and profile-isolated Phase 10 console probe with `APPDATA`/`LOCALAPPDATA` redirected under `.codex-research`.
- Output or parser errors: `--check-only --script res://addons/waterways/ripple_gizmo.gd` exited cleanly. `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` passed and reported `emitter_gizmo_handles={moving_handles=2, pulse_handles=1}` plus `emitter_handle_model={radius_property=radius, moving_property=moving_emit_distance, moving_ids=2, pulse_ids=1}`.
- Visible result, if applicable: Not rerun human-visibly at that moment; the later tiny-grip result above confirms core drag/undo/redo behavior passed after this material fix.
- Stable result marker: `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`
- Pass/partial/fail: Historical partial pass: material lookup fix was automated-proven here; later practical handle review passed.
- Notes or follow-up: Console probes must use repo-local `APPDATA`/`LOCALAPPDATA` isolation. A `--user-data-dir` attempt crashed Godot locally and was not used as evidence.
...
- Ran by: Agent
- Godot version/renderer/device: Godot 4.6.3 stable, Vulkan Forward+, AMD Radeon RX 6800 XT
- Command, scene, or workflow: Added the Phase 10 emitter handle contract/checklist, parser checks for `ripple_gizmo_handle_model.gd`, `ripple_gizmo_geometry.gd`, `ripple_gizmo.gd`, `plugin.gd`, and `ripple_phase10_inspector_probe.gd`; profile-isolated console probe `--script res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_phase10_inspector_probe.gd`; clean serial regression probes `ripple_phase9_preset_api_probe.gd`, `ripple_field_emitter_probe.gd`, `ripple_material_ownership_probe.gd`, and `ripple_field_emitter_demo_review_probe.gd`; static `git diff --check`, forbidden-call scan, and runtime editor-class/save-path scan.
- Output or parser errors: Stable markers `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`, `RIPPLE_PHASE9_PRESET_API_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PROBE_OK`, `RIPPLE_MATERIAL_OWNERSHIP_PROBE_OK`, and `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_PROBE_OK`. The Phase 10 probe reported `emitter_gizmo_handles={moving_handles=2, pulse_handles=1}` and `emitter_handle_model={radius_property=radius, moving_property=moving_emit_distance, moving_ids=2, pulse_ids=1}`. `git diff --check` passed with the existing unrelated CRLF warning for `addons/zylann.hterrain/util/quad_tree_lod.gd`. The material-ownership probe printed expected rejected-key/wrong-owner warnings while still passing.
- Visible result, if applicable: Not human-visible at that time; later recorded results above confirm practical handle behavior passed human-visible review, while no-op/cancel remained unchecked and dirty-prompt reporting was unclear.
- Stable result marker: `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`
- Pass/partial/fail: Historical partial pass for Phase 10 emitter handles: automated contract pass at that time; later practical handle review passed.
- Notes or follow-up: At that time, field `world_bounds` handles were still a future slice. The first handle slice edits only emitter `radius` and `moving_emit_distance` through editor undo and keeps runtime field/emitter scripts free of editor-only references.
...
- Ran by: Agent
- Godot version/renderer/device: Godot 4.6.3 stable, Vulkan Forward+, AMD Radeon RX 6800 XT
- Command, scene, or workflow: Profile-isolated parser checks for `ripple_gizmo_geometry.gd`, `ripple_gizmo.gd`, and `plugin.gd`; profile-isolated console probe `--script res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_phase10_inspector_probe.gd`; clean serial regression probes `ripple_phase9_preset_api_probe.gd`, `ripple_field_emitter_probe.gd`, `ripple_material_ownership_probe.gd`, and `ripple_field_emitter_demo_review_probe.gd`; static `git diff --check`, forbidden-call scan, and runtime editor-class/save-path scan.
- Output or parser errors: Stable markers `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`, `RIPPLE_PHASE9_PRESET_API_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PROBE_OK`, `RIPPLE_MATERIAL_OWNERSHIP_PROBE_OK`, and `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_PROBE_OK`. The material-ownership probe printed its expected invalid-key/wrong-owner warnings while still passing. `git diff --check` passed with an unrelated CRLF warning for `addons/zylann.hterrain/util/quad_tree_lod.gd`.
- Visible result, if applicable: Not human-visible; this validates the visual-only gizmo contract by generated helper segment counts and static scans.
- Stable result marker: `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`
- Pass/partial/fail: Pass for Phase 10 visual gizmo helpers at the automated-contract stage; human-visible editor gizmo readability/dirty-state review later passed.
- Notes or follow-up: At this visual-only point, the Phase 10 probe reported `field_gizmo_segments={bounds=24, footprint=12, routes=2}` and `emitter_gizmo_segments={radius=128, moving=64, route=2}`. That slice added no handles, no preview children, no exported-value mutation, no runtime material/boundary/emission calls, and no runtime editor/save leakage. The later emitter handle slice is recorded above.
...
- Command, scene, or workflow: Human-visible Phase 10 gizmo review in `res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_field_emitter_demo_review.tscn`: selected `WaterRippleField`, selected `PulseEmitter_UpperWater`, `OneShotEmitter_MidBend`, and `MovingEmitter_TestTrail`, switched selections, checked plugin/scene lifecycle behavior, and ran/stopped the scene once.
- Output or parser errors: User reported no Output panel errors.
- Visible result, if applicable: User confirmed steps 3-7 passed. Field box, footprint, and route helpers were readable. Emitter radius, moving-threshold, and route helpers were readable. Viewing only did not produce an unsaved-scene prompt.
- Stable result marker: Human-visible Phase 10 visual gizmo review.
- Pass/partial/fail: Pass for human-visible Phase 10 visual gizmo readability, dirty-state, lifecycle, play-stop cleanup, and no-Output-error behavior.
- Notes or follow-up: This closes the visible review gate for the visual-only gizmo helper slice. At that point editable handles, boundary previews, `debug_visible` re-exposure, live-scene commands, and response/refraction/displacement tuning remained separate future slices; the later emitter handle slice is recorded above.
...
- Ran by: User
- Godot version/renderer/device: Godot 4.6.3 editor, local project; renderer/device not relayed.
- Command, scene, or workflow: Human-visible Phase 10 capture/save review in `res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_field_emitter_demo_review.tscn`: Waterways enabled, selected `WaterRippleField`, clicked `Capture Field`, saved through `Save Captured` to a deliberate scratch `.tres` path, repeated with one `WaterRippleEmitter`, restarted/reopened the editor, loaded the saved presets, and checked for node preset slots, paths, live links, or auto-apply behavior.
- Output or parser errors: User reported no Output errors.
- Visible result, if applicable: User completed the checklist: capture alone did not dirty the scene; saved field/emitter presets loaded after restart/reopen with the expected types and values; and no field/emitter node gained a preset slot, resource path, live link, or auto-apply behavior.
- Stable result marker: Human-visible Phase 10 capture/save review.
- Pass/partial/fail: Pass for human-visible Phase 10 capture/save controls.
- Notes or follow-up: This closes the visible save-dialog, dirty-state, restart/reload, and no-live-link gate for Phase 10 capture/save. Visual-only gizmo helpers are now automated- and human-visible-proven; at that point editable handles, boundary previews, `debug_visible` re-exposure, live-scene commands, and any response/refraction/displacement tuning remained separate future slices. The later emitter handle slice is recorded above.
...
- Notes or follow-up: The checklist now clarifies that "no-op apply" means applying the same actual preset again after the node already matches it, not selecting the neutral `Choose...` placeholder. Capture/save was later implemented, automated-proven, and human-visible accepted.
...
- Notes or follow-up: This retires the visible lifecycle gate for the non-mutating inspector skeleton. Transient preset selectors, undo-aware Apply, capture/save, and visual-only gizmos were later human-visible accepted. Boundary previews and live-scene commands remain future validation work.
...
- Notes or follow-up: `ripple_inspector_plugin.gd` is registered and removed by `plugin.gd`, while `ripple_inspector_status.gd` holds probeable read-only status logic. The status model reports exported values, configuration warnings, and conservative target/routing summaries only, avoids runtime/private resolver calls, adds no children, mutates no field/emitter values, and originally left all mutating editor actions and visual helpers for later Phase 10 slices. Preset selectors, Apply, capture/save, visual-only gizmos, handles, and automated boundary preview are now implemented in later slices; live-scene commands remain deferred.
...
## Phase 10 Gizmo Human Review Request

Use this exact request when rerunning the visual helper readability review. Current builds may also show draggable handles and the field boundary preview; handle-specific behavior is covered by the handle review requests below.

1. Open Godot 4.6.3 with the project at `C:\Users\pc\Documents\GitHub\Godot 4 Waterways Demo`.
2. Confirm the Waterways plugin is enabled, then open `res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_field_emitter_demo_review.tscn`.
3. In the editor 3D viewport, select `WaterRippleField`. Confirm a cyan field bounds box appears around the ripple domain, a green X/Z footprint rectangle and center cross are visible, a magenta projected boundary/river-footprint outline appears on the field plane, and a faint route line points toward the configured target river.
4. Select each emitter child: `PulseEmitter_UpperWater`, `OneShotEmitter_MidBend`, and `MovingEmitter_TestTrail`. Confirm orange radius rings appear around each emitter. For `MovingEmitter_TestTrail`, also confirm a dashed yellow moving-threshold ring appears. A faint route line should point back toward the field.
5. Change selection away from the ripple nodes and back again. Disable and re-enable the Waterways plugin if comfortable. Close/reopen the scene. The helper lines should appear only through the editor gizmo system, with no extra preview nodes left in the scene tree and no Output errors.
6. Confirm selecting, viewing, switching selections, and plugin enable/disable does not prompt for unsaved scene changes. Do not save the scene unless you intentionally changed a normal property.
7. Run and stop the scene once. Confirm the existing field/emitter runtime review still behaves as before and the editor does not show stale helper lines or Output errors after stopping.
8. Relay renderer/device if visible, whether the field box/footprint/boundary-preview/route and emitter radius/moving/route helpers were readable, whether the boundary preview looked like a river footprint rather than a full square field, whether any unsaved-scene prompt appeared from viewing only, and any Output panel errors.

## Phase 10 Emitter Handle Human Review Request

Use this exact request when rerunning the emitter-handle review. Field bounds handles are covered by the separate request below.

1. Open Godot 4.6.3 with the project at `C:\Users\pc\Documents\GitHub\Godot 4 Waterways Demo`.
2. Confirm the Waterways plugin is enabled, then open `res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_field_emitter_demo_review.tscn`.
3. Select `PulseEmitter_UpperWater` in the editor 3D viewport or scene tree. Confirm the orange radius ring still appears and now has one draggable radius handle. Viewing/selecting only should not prompt for unsaved changes.
4. Drag the radius handle to a visibly different size. Confirm only the emitter `radius` property changes in the inspector; target routing, field settings, presets, bakes, `debug_visible`, `refraction_strength`, and `displacement_strength` should not appear or change. Undo and redo once, confirming the ring and inspector value follow the undo stack.
5. Select `MovingEmitter_TestTrail`. Confirm it shows one radius handle plus a moving-threshold grip. Because this review emitter uses a tiny dense-trail threshold (`moving_emit_distance = 0.08`), the true dashed yellow ring stays close to the emitter and a short yellow guide points to the more pickable moving-threshold grip. Drag that grip and confirm only `moving_emit_distance` changes. Undo and redo once.
6. Do a no-op check by selecting a handle without moving it, or dragging back to the same value if practical. Confirm no extra undo step or dirty prompt is created for an unchanged value.
7. If your Godot editor supports canceling a gizmo drag with Escape or right-click, start a drag, cancel it, and confirm the property value and ring return to the previous value with no new undo step. If the editor does not expose a cancel gesture, report that instead of forcing it.
8. Disable and re-enable the Waterways plugin, close/reopen the scene without saving, and delete/undo-delete one selected emitter if comfortable. Confirm no stale handles, preview children, scratch resources, Output errors, or unexpected scene changes remain.
9. Run and stop the scene once. Confirm the existing field/emitter runtime review still behaves as before and stopping does not leave stale handles or runtime material state.
10. Relay renderer/device if visible, the before/after `radius` and `moving_emit_distance` values you edited, whether undo/redo worked, whether no-op/cancel behaved as expected, whether any unsaved-scene prompt appeared from viewing only, and any Output panel errors.

## Phase 10 Field Bounds Handle Human Review Request

Use this exact request after undo-backed field `world_bounds` handles are implemented.

1. Open Godot 4.6.3 with the project at `C:\Users\pc\Documents\GitHub\Godot 4 Waterways Demo`.
2. Confirm the Waterways plugin is enabled, then open `res://addons/waterways/docs/spec-driven/features/river-ripples/probes/ripple_field_emitter_demo_review.tscn`.
3. Select `WaterRippleField` in the editor 3D viewport or scene tree. Confirm the cyan bounds box, green X/Z footprint, and route helper still appear, and that the field now has six draggable face handles: min/max X, min/max Y, and min/max Z.
4. Before dragging, note the current `world_bounds` position and size from the inspector. Viewing/selecting only should not prompt for unsaved changes.
5. Drag one horizontal X or Z face handle to make the field visibly wider or narrower. Confirm only the field `world_bounds` property changes; target routing, boundary settings, presets, bakes, `debug_visible`, `refraction_strength`, `displacement_strength`, emitter properties, and river/WaterSystem settings should not appear or change. Undo and redo once, confirming the box, footprint, and inspector value follow the undo stack.
6. Drag the opposite face on the same axis, then undo and redo once. Confirm the first face stays fixed while the dragged face moves.
7. If practical, drag one Y face handle up or down and undo/redo once. This only changes the vertical AABB span; it should not run the simulation or change visible runtime river state in the editor.
8. Do a no-op check by selecting a field handle without moving it, or dragging back to the same value if practical. Confirm no extra undo step or dirty prompt is created for an unchanged value.
9. If your Godot editor supports canceling a gizmo drag with Escape or right-click, start a drag, cancel it, and confirm `world_bounds` and the box return to the previous value with no new undo step. If the editor does not expose a cancel gesture, report that instead of forcing it.
10. Disable and re-enable the Waterways plugin, close/reopen the scene without saving, and delete/undo-delete the selected field if comfortable. Confirm no stale handles, preview children, scratch resources, Output errors, or unexpected scene changes remain.
11. Run and stop the scene once. Confirm the existing field/emitter runtime review still behaves as before and stopping does not leave stale handles or runtime material state.
12. Relay renderer/device if visible, the before/after `world_bounds` values you edited, whether undo/redo worked, whether no-op/cancel behaved as expected, whether any unsaved-scene prompt appeared from viewing only, and any Output panel errors.
