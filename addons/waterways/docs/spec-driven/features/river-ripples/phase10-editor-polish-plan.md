...
1. Inspector plugin skeleton plus read-only status rows for field/emitter nodes. This retires the editor-load and lifecycle risks first. It may show warnings, routing/target summaries, and safe snapshots, but must not mutate scene/runtime state or expose preset/save/preview buttons yet.
2. Plugin lifecycle validation: enable/disable, inspector reselection, scene close/reopen, play/stop, and Add Node visibility checks with no Output errors and no stale UI. This proves registration, unregistration, and scratch cleanup before mutation exists.
3. Undo-aware apply-preset buttons or selector actions for already-defined Phase 9 resources. This retires the copied-value and dirty-state risks. It must use transient selector state, no live profile link, no serialized preset slot, per-property undo operations, and no-op apply skipping.
4. Capture/save preset buttons with explicit editor-only resource save behavior. This retires the `ResourceSaver` leakage risk. It must avoid automatic runtime writeback, source-preset overwrite defaults, and scene dirtying unless a separate intentional assignment action is added to the matrix.
5. Optional bounds/radius gizmos only if they use editor undo/redo and do not touch runtime materials, bakes, `WaterSystem` data, source signatures, or buoyancy.
