...
- Editor tools:
  ...
  - Phase 10 visual gizmo scripts: `addons/waterways/ripple_gizmo.gd` registered from `addons/waterways/plugin.gd`, `addons/waterways/ripple_gizmo_geometry.gd` for probeable field-bounds/footprint/route and emitter radius/moving-threshold/route helpers plus handle positions, and `addons/waterways/ripple_gizmo_handle_model.gd` for probeable emitter handle IDs, property mapping, clamping, and no-op diffs.
