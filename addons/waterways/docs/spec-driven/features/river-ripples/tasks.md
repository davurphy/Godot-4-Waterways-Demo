...
- Current status: Phase 0 decisions locked; standalone feedback, mapping, mesh-footprint boundary-mask, RiverManager-facing material ownership/restore, minimal river shader neutral-path, river shader sampling-budget, visible normal render, demo review-scene smoke, demo review-scene diagnostic, debug parity, shader-cost, reusable field/emitter lifecycle, field/emitter dispatch-performance, post-review-strength-boost terrain-aware demo-backed field/emitter authoring, Mobile/Compatibility renderer probes, Phase 10 read-only inspector skeleton, Phase 10 transient preset apply controls, Phase 10 capture/save controls, Phase 10 visual field/emitter gizmo helpers, first undo-backed emitter handle slice, undo-backed field `world_bounds` face handle slice, and editor-only field boundary preview slice pass automated validation; feedback, mapping, boundary mask, demo-backed minimal visible normal blend, demo-backed debug parity, demo-backed field/emitter visual/debug/control/stop-reload behavior, Add Node visibility for `WaterRippleField`/`WaterRippleEmitter`, the Phase 9 editor authoring surface, Phase 10 read-only inspector lifecycle/dirty-state review, Phase 10 Apply active-preset/undo/no-op/boundary/lifecycle review, Phase 10 capture/save save-dialog/dirty-state/restart/no-live-link review, Phase 10 visual gizmo readability/dirty-state/lifecycle review, Phase 10 emitter radius/moving-threshold drag/undo/redo/lifecycle review, Phase 10 field `world_bounds` handle review, and Phase 10 boundary preview review also passed human-visible validation.
...
- Remaining open task focus: Phase 9 script/resource authoring is accepted; Phase 10 read-only inspector, transient preset Apply controls, capture/save controls, visual gizmo helpers, practical emitter handle behavior, field `world_bounds` handle behavior, and boundary preview behavior are automated- and human-visible-proven. Emitter handle no-op/cancel was not checked because Godot did not expose an obvious gesture, and dirty-prompt reporting was unclear, but neither blocks the handle slice. `debug_visible` re-exposure, live-scene bridge work, and later flow-aware or physics-facing work remain deferred.
- Last passing validation: Godot 4.6.3 Forward+ console `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`, parser checks for `ripple_gizmo_handle_model.gd`, `ripple_gizmo_geometry.gd`, `ripple_gizmo.gd`, and `ripple_phase10_inspector_probe.gd`, plus prior parser checks for `ripple_inspector_preset_apply_model.gd`, `ripple_inspector_status.gd`, `ripple_inspector_plugin.gd`, and `plugin.gd` passed on 2026-06-08. Latest Phase 10 validation includes active preset status/selector restoration, capture/save checks, visual gizmo checks, boundary preview checks, emitter handle checks, and field bounds handle checks: a projected mesh-footprint boundary preview outline of `8` segment points for a two-triangle quad, six `world_bounds` face handles with stable IDs `2001..2006`, face-center positions, `world_bounds`-only property mapping, opposite-face preservation, no-op diff skipping, min-size clamping, no preview/runtime children, no exported-value mutation while building helpers, moving emitters expose radius plus moving-threshold handles, non-moving emitters expose only radius, tiny moving-threshold pickability guide/grip still works, and no forbidden runtime/editor leakage exists. Post-boundary-preview regression probes passed for Phase 9 preset API, field/emitter lifecycle, material ownership, and the demo review scene; static scans found no forbidden editor helper calls or runtime editor/save leakage. User confirmed the Phase 10 read-only inspector lifecycle/dirty-state review, Phase 10 Apply review, Phase 10 capture/save review, Phase 10 visual gizmo review, practical emitter handle review, field `world_bounds` handle editor tests, and boundary preview tests.
- Next recommended action: Choose `debug_visible` re-exposure design, live-scene bridge planning, or a separate plan for response/refraction/displacement tuning. Final flow, WaterSystem, bakes, source signatures, buoyancy, and physics-facing ripple behavior remain deferred.
...
- [x] Phase 10 implementation slice 3: Add capture/save preset workflow.
  ...
  - Current result: Implemented `Capture Field` / `Capture Emitter` and `Save Captured` controls in `ripple_inspector_plugin.gd`. Capture calls the existing in-memory `capture_preset()` path, stores the result as transient inspector/plugin state, starts with an empty `resource_path`, and does not assign a preset slot to the node. Save opens an editor-owned `EditorFileDialog` in `res://` save-file mode with a `.tres` filter and calls `ResourceSaver.save()` only after a user path is selected. `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` validates capture non-mutation, saved scratch `.tres` reload, no target/reserved value persistence, explicit save paths, and no editor/save leakage into runtime scripts or preset resources. User confirmed the visible save-dialog, dirty-state, restart/reload, and no-live-link review passed with no Output errors.

- [x] Phase 10 implementation slice 4: Add visual-only field/emitter gizmo helpers.
  - Validate: Helper previews are editor-only gizmos or non-persistent scratch state; exported runtime scenes do not depend on them; no handles, saved toggles, runtime material calls, bakes, `WaterSystem`, source signatures, or buoyancy changes are introduced.
  - Current result: Added `ripple_gizmo.gd` registered from `plugin.gd` plus `ripple_gizmo_geometry.gd` as a probeable non-editor geometry helper. The gizmo draws field bounds, field X/Z footprint and center cross, explicit field target route lines, emitter radius rings, moving-emitter threshold rings, and emitter-to-field route lines. At this visual-only slice, `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` validated segment counts, no preview children, no exported-value mutation, forbidden-call boundaries, plugin registration/removal, and no runtime editor/save leakage. The later emitter handle slice is recorded below.

- [x] Phase 10 human-visible gizmo review.
  - Current result: User confirmed steps 3-7 passed. Field box/footprint/route helpers and emitter radius/moving/route helpers were readable, viewing only did not prompt for unsaved scene changes, and Output showed no errors.

- [x] Phase 10 handle planning: Define the undo-backed gizmo handle contract before implementation.
  - Validate: `phase10-editor-polish-plan.md` specifies mutation, undo/redo, dirty-state, no-op, cancel, plugin disable, scene close/reopen, and play/stop behavior for emitter radius/moving-threshold handles and planned field `world_bounds` face handles; `validation.md` includes the human-visible handle checklist.
  - Current result: Contract and checklist are documented. The first implementation slice covered emitter handles; the later field handle slice uses the same undo/dirty-state contract.

- [x] Phase 10 implementation slice 5: Add undo-backed emitter radius and moving-threshold handles.
  - Validate: Handles edit only exported emitter properties through `EditorUndoRedoManager`, skip no-op diffs, clamp to exported ranges, add no preview children, call no runtime rebuild/reset/emission/material/boundary paths, and keep runtime scripts free of editor-only references.
  - Current result: Added `ripple_gizmo_handle_model.gd` as a probeable handle contract helper; `ripple_gizmo_geometry.gd` now exposes handle positions; `ripple_gizmo.gd` adds axis-constrained emitter handles and per-property undo commits; `plugin.gd` passes/clears editor undo for the ripple gizmo. `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` validates moving emitter handles `2`, pulse emitter handles `1`, stable handle IDs, property whitelist, clamping, no-op diff skipping, no preview children, and forbidden-call/runtime-leak boundaries.

- [x] Phase 10 human-visible emitter handle practical review.
  - Validate: Run the `Phase 10 Emitter Handle Human Review Request` in `validation.md` for drag, undo, redo, no-op/cancel if supported, dirty-state behavior, plugin disable/re-enable, scene close/reopen, selected-node deletion, and play/stop cleanup.
  - Current result: Practical review passed after the tiny moving-threshold grip fix: user confirmed the radius handle changes only `radius`, the moving-threshold grip changes only `moving_emit_distance`, undo/redo works, plugin disable/re-enable works, close/reopen without saving works, delete/undo-delete works, run/stop cleanup works, and Output shows no errors. No-op/cancel was not checked because Godot did not expose an obvious gesture; dirty-prompt reporting was unclear, not a blocker.

- [x] Phase 10 implementation slice 6: Add undo-backed field `world_bounds` face handles.
  - Validate: Six face handles edit only `world_bounds` through `EditorUndoRedoManager`, preserve the opposite AABB face, clamp before flipping below the minimum size, skip no-op diffs, add no preview children, call no runtime rebuild/reset/emission/material/boundary paths from editor helper code, and keep runtime scripts free of editor-only references.
  - Current result: `ripple_gizmo_handle_model.gd` now exposes stable field face IDs `2001..2006`, `ripple_gizmo_geometry.gd` places handles at the six AABB face centers, and `ripple_gizmo.gd` projects drags along world X/Y/Z axes before committing one undoable `world_bounds` property change. `RIPPLE_PHASE10_INSPECTOR_PROBE_OK` validates six field handles, property mapping, face positions, opposite-face preservation, min-size clamping, no-op diff skipping, and forbidden-call/runtime-leak boundaries.

- [x] Phase 10 human-visible field `world_bounds` handle review.
  - Validate: User checks min/max X/Y/Z face drags change only `world_bounds`, undo/redo, no-op/cancel if Godot exposes a gesture, plugin disable/re-enable, scene close/reopen, play/stop behavior, and no Output errors.
  - Current result: User confirmed the field `world_bounds` handle tests all pass cleanly in the editor.

- [x] Phase 10 optional helper: Add editor-only field boundary preview.
  - Validate: The preview is an editor-only gizmo outline built from configured boundary/target mesh sources; it creates no preview children, runs no runtime boundary generation or material calls, mutates no exported values, keeps runtime scripts free of editor-only references, and passes `RIPPLE_PHASE10_INSPECTOR_PROBE_OK`.
  - Current result: `ripple_gizmo_geometry.gd` projects target/boundary mesh footprint edges onto the field's X/Z plane and `ripple_gizmo.gd` draws them with a dedicated material. The probe verifies a two-triangle quad becomes a four-edge `field_boundary_preview` outline with `8` segment points, with no runtime/editor leakage; Phase 9, field/emitter, material ownership, and demo review regressions passed.

- [x] Phase 10 human-visible boundary preview review.
  - Validate: User checks the magenta projected footprint outline in the editor, confirms it is readable relative to field bounds/footprint/route helpers, confirms viewing does not dirty the scene, and confirms plugin lifecycle plus play/stop leave no stale helpers or Output errors.
  - Current result: User reported the tests all pass.

- [ ] Future Phase 10 optional helpers: Add `debug_visible` re-exposure only after its matrix row is accepted.
  - Validate: Helper previews are editor-only overlays/gizmos or non-persistent scratch state; exported runtime scenes do not depend on them; cleanup runs on plugin disable, scene close, node deletion, inspector reselection, and play/stop; `debug_visible` is re-exposed or renamed only after visible helper behavior and saved-toggle semantics are accepted; `refraction_strength` and `displacement_strength` stay out of Phase 10.
