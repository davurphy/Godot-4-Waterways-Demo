...
- Current status: Phase 0 decisions locked; standalone feedback, mapping, mesh-footprint boundary-mask, RiverManager-facing material ownership/restore, minimal river shader neutral-path, river shader sampling-budget, visible normal render, demo review-scene smoke, demo review-scene diagnostic, debug parity, shader-cost, reusable field/emitter lifecycle, field/emitter dispatch-performance, post-review-strength-boost terrain-aware demo-backed field/emitter authoring, and Mobile/Compatibility renderer probes pass automated validation; feedback, mapping, boundary mask, demo-backed minimal visible normal blend, demo-backed debug parity, demo-backed field/emitter visual/debug/control/stop-reload behavior, Add Node visibility for `WaterRippleField`/`WaterRippleEmitter`, and the Phase 9 editor authoring surface also passed human-visible validation.
...
- Remaining open task focus: Phase 9 script/resource authoring is accepted; next choose between Phase 10 native editor polish or a separate response/refraction/displacement tuning plan. Later flow-aware or physics-facing work remains deferred.
- Last passing validation: Godot 4.6.3 Forward+ console `RIPPLE_PHASE9_PRESET_API_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PROBE_OK`, `RIPPLE_MATERIAL_OWNERSHIP_PROBE_OK`, and `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_PROBE_OK` passed for the Phase 9 authoring grouping/built-in preset/save-reload slice on 2026-06-08. Parser checks passed for the two preset resources, `water_ripple_field.gd`, `water_ripple_emitter.gd`, and `ripple_phase9_preset_api_probe.gd`; static `git diff --check`, forbidden-file diff, and runtime preset-save/readback scans stayed clean. User then confirmed the Phase 9 editor authoring review: field/emitter inspector groups were readable, reserved fields stayed hidden, Add Node search found both ripple nodes by name/icon, the review scene still behaved correctly, and Output showed no errors. Earlier Mobile and Compatibility Godot 4.6.3 console `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_PROBE_OK`, `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_DIAGNOSTIC_OK`, `RIPPLE_DEBUG_PARITY_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PERFORMANCE_PROBE_OK`, and `RIPPLE_SHADER_COST_PROBE_OK` also passed.
- Next recommended action: Choose and plan the next implementation slice. Use `phase10-editor-polish-plan.md` for native editor buttons, undo-aware editor actions, save dialogs, and editor preview/debug helpers; use a separate plan for response/refraction/displacement tuning. Final flow, WaterSystem, bakes, source signatures, buoyancy, and physics-facing ripple behavior remain deferred.
...
- [ ] Phase 10 planning: Native ripple editor polish.
  - Validate: `phase10-editor-polish-plan.md` remains the current plan for inspector buttons, undo-aware editor actions, save dialogs, and editor-safe preview/debug helpers after Phase 9 resources/APIs are stable.
  - Current result: Phase 10 is documented separately so Phase 9 does not over-promise editor buttons or editor-time simulation preview.

- [ ] Phase 10 implementation: Add dedicated ripple inspector tooling only after Phase 9 stabilizes.
  - Validate: Editor plugin enable/disable is clean, editor preset apply is undoable/redoable, captured presets save/load explicitly, scene dirty state changes only when expected, and exported runtime scenes do not depend on editor-only nodes/classes.
