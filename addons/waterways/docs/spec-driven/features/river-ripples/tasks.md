...
- Current status: Phase 0 decisions locked; standalone feedback, mapping, mesh-footprint boundary-mask, RiverManager-facing material ownership/restore, minimal river shader neutral-path, river shader sampling-budget, visible normal render, demo review-scene smoke, demo review-scene diagnostic, debug parity, shader-cost, reusable field/emitter lifecycle, field/emitter dispatch-performance, and post-clear-fix terrain-aware demo-backed field/emitter authoring probes pass automated validation; feedback, mapping, boundary mask, demo-backed minimal visible normal blend, and demo-backed debug parity also passed human-visible validation.
- Current implementation slice: prototype `water_ripple_field.gd` / `water_ripple_field.tscn` owns runtime ping-pong simulation, impulse, and boundary-mask `SubViewport` textures and applies owner-scoped `i_ripple_*` state through RiverManager; prototype `water_ripple_emitter.gd` / `water_ripple_emitter.tscn` supports pulse, continuous, one-shot, and moving impulse modes. The river shader slice remains minimal normal-only; custom refraction and displacement remain untuned/off.
- Remaining open task focus: human-visible rerun of the terrain-aware field/emitter authoring workflow, non-Forward+ renderer coverage, and later public API polish.
- Last passing validation: post-clear-fix Godot 4.6.3 console `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_DIAGNOSTIC_OK`, terrain-aware `RIPPLE_FIELD_EMITTER_DEMO_REVIEW_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PERFORMANCE_PROBE_OK`, `RIPPLE_FIELD_EMITTER_PROBE_OK`, and regression `RIPPLE_SHADER_COST_PROBE_OK`, `RIPPLE_DEBUG_PARITY_PROBE_OK`, `RIPPLE_MATERIAL_OWNERSHIP_PROBE_OK`, `RIPPLE_RIVER_SHADER_NEUTRAL_PROBE_OK`, `RIPPLE_RIVER_SHADER_SAMPLING_PROBE_OK`, `RIPPLE_RIVER_SHADER_VISIBLE_NORMAL_PROBE_OK`, `RIPPLE_RIVER_SHADER_VISIBLE_NORMAL_REVIEW_PROBE_OK`, `RIPPLE_RIVER_SHADER_VISIBLE_NORMAL_REVIEW_DIAGNOSTIC_OK`, `RIPPLE_BOUNDARY_MASK_PROBE_OK`, `RIPPLE_MAPPING_PROBE_OK`, `RIPPLE_FEEDBACK_PROBE_OK`, and `RIPPLE_FEEDBACK_ANALYSIS_OK`, plus user-visible `ripple_boundary_mask_review.tscn`, `ripple_mapping_review.tscn`, `ripple_feedback_review.tscn`, and demo-backed `ripple_river_shader_visible_normal_review.tscn` reviews on 2026-06-07.
- Next recommended action: Ask for human-visible rerun of `ripple_field_emitter_demo_review.tscn`, specifically checking that emitter markers/handles sit above exposed river water and that F pulses land underneath them; keep response/refraction/displacement tuning deferred until the reusable authoring workflow is accepted.
...
- [x] Phase 7: Add debug and validation probes.
  ...
  - Current result: Existing feedback, mapping, boundary, material ownership, visible/debug shader, and shader-cost probes remain passing; `ripple_field_emitter_probe.gd`, `ripple_field_emitter_performance_probe.gd`, `ripple_field_emitter_demo_review_probe.gd`, and `ripple_field_emitter_demo_review_diagnostic.gd` cover prototype field/emitter lifecycle, dispatch cost, demo placement/cleanup, and zero-clear impulse texture behavior. Static scans stayed clean for normal-runtime readback and forbidden bake/WaterSystem/source-signature/buoyancy file changes.
...
- [x] Phase 8: Measure full field/emitter performance and tune defaults.
  ...
  - Current result: `RIPPLE_FIELD_EMITTER_PERFORMANCE_PROBE_OK` passed on Godot 4.6.3 Forward+ / AMD Radeon RX 6800 XT for 128/256/512 fields with 0/4/16 emitters after the transparent-zero viewport clear fix. Dispatch medians were `5.477/6.059/5.704 ms` at 128, `6.405/5.650/5.521 ms` at 256, and `5.754/5.508/5.377 ms` at 512; current shader disabled/enabled timing is covered by `RIPPLE_SHADER_COST_PROBE_OK`. Non-Forward+ and human-visible demo authoring review remain open.
