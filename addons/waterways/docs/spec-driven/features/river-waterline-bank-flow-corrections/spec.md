...
The missing-water premise is now confirmed and fixed: top-down/down-ray collision occupancy could still mark open waterline under overhangs as solid. The bank-flow premise remains unverified and should still be treated as a diagnosis task.
...
- Status: waterline missing-water issue fixed and visually validated after user rebake; bank-flow issue remains open.
- Source of truth for open work: `tasks.md` "Open Work".
- Last meaningful decision: add waterline-contact gating to collision occupancy and bump the bake signature; user rebake confirmed the reported missing-water gaps are resolved.
- Known deferred items: bank-flow classification, possible bank-flow probe/fix, and any generated-resource hygiene the user wants after the rebake.
...
- Keep the reported missing-water gaps fixed by requiring actual waterline contact before upper/top-down collider hits can remove water.
...
- User-reported observations:
  ...
  - Three red-circled missing-water screenshots were supplied and later confirmed fixed after the user rebaked the river.
- Agent confidence in the waterline premise: confirmed and fixed. Agent confidence in the bank-flow premise: medium until a specific bank location is probed.
...
- Status: pass for the reported overhang screenshots after the waterline-contact collision occupancy patch and user rebake.
...
- AT5: After any fix and rebake, reported obstacle gap locations show water where the waterline is open and no water inside true solids. Status: reported obstacle gaps show water after user rebake; no new true-solid issue reported.
...
| Are the obstacle gaps present after a fresh rebake on current code? | No for the reported red-circled locations. | 2026-06-16 | User rebaked the river after the patch and reported the missing water is resolved. |
...
| 2026-06-16 | Mark the missing-water issue visually resolved after user rebake. | User rebaked the affected river after the patch and confirmed the red-circled gaps were gone. |
