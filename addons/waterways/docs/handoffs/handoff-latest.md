...
Feature-local spec-driven docs now exist at `addons/waterways/docs/spec-driven/features/river-pillows/`. For pillow work, treat that folder as the current source of truth before returning to this shared handoff; the pillow-relevant roadmap and audit guidance has been folded into that feature folder. The tooltip/field-description work is not complete. The next pillow step was a signature-`20` rebake and live review, but the next session must first fix the new editor regression errors reported after the failed tooltip session.

Regression note for the next session: the user reports errors that were not present before the tooltip session, including `res://addons/zylann.hterrain/tools/packed_textures/packed_texture_importer.gd:93 - Parse Error: Expected closing ")" after call arguments.` These regression errors need to be fixed by the next session before any more pillow, tooltip, bake, or shader work.
...
Recommended next move: fix the new editor regression errors first, including the hterrain packed-texture importer parse error. After the editor is stable again, regenerate the main and obstacle-test river bakes to signature `20`, then review the same pillow targets with the raw/final/source-term debug views. Keep `pillow_forward_reach_tiles = 0.0` while reviewing and leave WaterSystem/physics data untouched.
